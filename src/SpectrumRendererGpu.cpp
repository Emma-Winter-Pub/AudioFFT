...
void SpectrumRendererGpu::updateVertexBuffer() {
    ...
    if (m_type == SpectrumProfileType::Bar) {
        ...
        for (size_t i = 0; i < count; ++i) {
            float pos = static_cast<float>(i) / (count - 1);
            float val = m_dataCache[i];
            if (val > 1.0f) val = 1.0f;
            if (val < 0.0f) val = 0.0f;
            if (isHorz) {
                m_vertexBufferData.push_back(pos); m_vertexBufferData.push_back(0.0f);
                m_vertexBufferData.push_back(pos); m_vertexBufferData.push_back(val);
            } else {
                m_vertexBufferData.push_back(0.0f); m_vertexBufferData.push_back(pos);
                m_vertexBufferData.push_back(val);  m_vertexBufferData.push_back(pos);
            }
        }
        ...
        for (size_t i = 0; i < count; ++i) {
            ...
            if (isHorz) {
                m_vertexBufferData.push_back(pos - halfWidth); m_vertexBufferData.push_back(val);
                m_vertexBufferData.push_back(pos + halfWidth); m_vertexBufferData.push_back(val);
            } else {
                m_vertexBufferData.push_back(val); m_vertexBufferData.push_back(pos - halfWidth);
                m_vertexBufferData.push_back(val); m_vertexBufferData.push_back(pos + halfWidth);
            }
        }
