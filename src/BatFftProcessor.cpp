    ...
    const PcmData32& pcmData, double timeInterval, int fftSize, int sampleRate, WindowType windowType)
{
    ...
    while (currentFrameIdx < fullFrames) {
        ...
        for (int b = 0; b < framesToProcess; ++b) {
            ...
            for (int i = 0; i < binCount; ++i) {
                float mag = std::sqrt(frameOut[i][0] * frameOut[i][0] + frameOut[i][1] * frameOut[i][1]) / static_cast<float>(N);
                if (i > 0 && i < binCount - 1) mag *= 2.0f;
                dstPtr[outOffset + i] = 20.0f * std::log10(mag + 1e-9f);
            }
        }
    ...
    if (needPadding) {
        ...
        for (int i = 0; i < binCount; ++i) {
            float mag = std::sqrt(outBase[i][0] * outBase[i][0] + outBase[i][1] * outBase[i][1]) / static_cast<float>(N);
            if (i > 0 && i < binCount - 1) mag *= 2.0f;
            dstPtr[outOffset + i] = 20.0f * std::log10(mag + 1e-9f);
        }
    }
    ...
    const PcmData64& pcmData, double timeInterval, int fftSize, int sampleRate, WindowType windowType)
{
    ...
    while (currentFrameIdx < fullFrames) {
        ...
        for (int b = 0; b < framesToProcess; ++b) {
            ...
            size_t outOffset = (currentFrameIdx + b) * binCount;
            for (int i = 0; i < binCount; ++i) {
                double mag = std::sqrt(frameOut[i][0] * frameOut[i][0] + frameOut[i][1] * frameOut[i][1]) / static_cast<double>(N);
                if (i > 0 && i < binCount - 1) mag *= 2.0;
                dstPtr[outOffset + i] = 20.0 * std::log10(mag + 1e-9);
            }
        }
        currentFrameIdx += framesToProcess;
    }
    if (needPadding) {
        ...
        size_t outOffset = currentFrameIdx * binCount;
        for (int i = 0; i < binCount; ++i) {
            double mag = std::sqrt(outBase[i][0] * outBase[i][0] + outBase[i][1] * outBase[i][1]) / static_cast<double>(N);
            if (i > 0 && i < binCount - 1) mag *= 2.0;
            dstPtr[outOffset + i] = 20.0 * std::log10(mag + 1e-9);
        }
    }
