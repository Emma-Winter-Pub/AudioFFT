...
#include "BatchFullLoadFFmpegRAII.h"
#include "BatchFullLoadIoScheduler.h" 
#include "AudioCodecWeights.h"
...
BatchFullLoadProcessor::BatchFullLoadProcessor(QObject *parent) 
    : QObject(parent)
{
...
BatchFullLoadProcessor::~BatchFullLoadProcessor(){
...
std::optional<BatchTask> BatchFullLoadProcessor::claimNextTask(){
    if (m_isHddMode) {
        ...
        BatchTask task = m_bufferPool.dequeue();
        m_ioCV.notify_all(); 
        return task;
    } 
    else {
...
void BatchFullLoadProcessor::notifyLargeFileFinished(){
...
void BatchFullLoadProcessor::releaseReadBuffer(size_t size) {
    m_currentPoolBytes -= size;
    m_ioCV.notify_all();
}

void BatchFullLoadProcessor::enqueueWriteTask(BatchWriteTask task) {
...
void BatchFullLoadProcessor::scanDirectory(const BatchSettings& settings) {
    if (m_runningWorkers.load() > 0) {
        emit logMessage(BatchFullLoadUtils::getCurrentTimestamp() + tr(" [错误] 一个任务已在运行中 请等待其完成后再开始新任务"));
        return;
    }
    cleanup();
    ...
    emit batchStarted();
    emit logMessage(BatchFullLoadUtils::getCurrentTimestamp() + tr(" 准备开始任务"));
    emit logMessage(BatchFullLoadUtils::formatSettings(settings, true));
    emit logMessage("--------------------------------------------------");
    ...
    m_planFuture = QtConcurrent::run([=]() {
        return BatchFullLoadIoScheduler::analyze(settings.inputPath, settings.outputPath, threadCount);
    });
    emit logMessage(BatchFullLoadUtils::getCurrentTimestamp() + tr(" 初始化任务 开始扫描有效音频文件"));
    ...
    if (validFiles.isEmpty()) {
        emit logMessage(BatchFullLoadUtils::getCurrentTimestamp() + tr(" 未找到任何包含音频流的文件"));
    ...
    emit logMessage(BatchFullLoadUtils::getCurrentTimestamp() + QString(tr(" [扫描完毕] 共找到 %1 个有效音频文件")).arg(validFiles.size()));
    emit scanFinished(snapshot);
}

void BatchFullLoadProcessor::startProcessing(){
    if (m_stopFlag.load()) {
        emit batchStopped();
        return;
    }
    ...
    BatchExecutionPlan plan = m_planFuture.result();
    startWorkers(m_pendingFiles, m_currentSettings, plan);
}

void BatchFullLoadProcessor::startWorkers(QList<FileInfo>& allFiles, const BatchSettings& settings, BatchExecutionPlan plan) {
    ...
    if (totalThreadQuota == 1) {
        plan.ioMode = BatchIoThreadMode::None;
        plan.workerDirectWrite = true;
    }
    emit logMessage(BatchFullLoadUtils::getCurrentTimestamp() + " " + plan.strategyName);
    int auxiliaryThreads = 0;
    if (plan.ioMode == BatchIoThreadMode::SeparateReadWrite) {
        auxiliaryThreads = 2;
    }
    else if (plan.ioMode != BatchIoThreadMode::None) {
        auxiliaryThreads = 1;
    }
    ...
    if (plan.ioMode == BatchIoThreadMode::ReaderOnly) {
        threadLog += tr("\n        I/O 线程: 1个读取");
    }
    else if (plan.ioMode == BatchIoThreadMode::WriterOnly) {
        threadLog += tr("\n        I/O 线程: 1个写入");
    }
    else if (plan.ioMode == BatchIoThreadMode::SeparateReadWrite) {
        threadLog += tr("\n        I/O 线程: 1个读取 + 1个写入");
    }
    else if (plan.ioMode == BatchIoThreadMode::Hybrid) {
        threadLog += tr("\n        I/O 线程: 1个混合读写");
    }
    if (plan.workerDirectWrite && plan.ioMode != BatchIoThreadMode::None) {
        threadLog += tr("\n        读写方式: 工作线程直接读写");
    }
    ...
    emit logMessage(BatchFullLoadUtils::getCurrentTimestamp() + tr(" 开始处理\n"));
    if (plan.ioMode == BatchIoThreadMode::None || plan.ioMode == BatchIoThreadMode::WriterOnly) {
    ...
    m_writeQueue.clear();
    if (plan.ioMode == BatchIoThreadMode::ReaderOnly || plan.ioMode == BatchIoThreadMode::SeparateReadWrite) {
        QThread* runner = QThread::create([this, allFiles]() {
            runReaderLoop(allFiles);
            });
        runner->setParent(this);
        connect(runner, &QThread::finished, this, &BatchFullLoadProcessor::onIoThreadFinished);
        m_ioThread = runner;
        runner->start();
    }
    if (plan.ioMode == BatchIoThreadMode::WriterOnly || plan.ioMode == BatchIoThreadMode::SeparateReadWrite) {
        QThread* runner = QThread::create([this]() {
            runWriterLoop();
            });
        runner->setParent(this);
        connect(runner, &QThread::finished, this, &BatchFullLoadProcessor::onWriterThreadFinished);
        m_writerThread = runner;
        runner->start();
    }
    if (plan.ioMode == BatchIoThreadMode::Hybrid) {
        QThread* runner = QThread::create([this, allFiles]() {
    ...
    bool managedRead = (plan.ioMode == BatchIoThreadMode::ReaderOnly ||
        plan.ioMode == BatchIoThreadMode::SeparateReadWrite ||
        plan.ioMode == BatchIoThreadMode::Hybrid);
    ...
    for (int i = 0; i < workerCount; ++i) {
        ...
        BatchFullLoadWorker* worker = new BatchFullLoadWorker(i + 1, m_bucketIdPadding, &m_stopFlag, &m_pauseFlag, this);
        worker->setDirectWriteMode(plan.workerDirectWrite);
        worker->moveToThread(thread);
        connect(thread, &QThread::finished, worker, &QObject::deleteLater);
        connect(thread, &QThread::started, this, [=]() {
            QMetaObject::invokeMethod(worker, "startWorkLoop", Qt::QueuedConnection, Q_ARG(BatchSettings, settings));
            });
        connect(worker, &BatchFullLoadWorker::fileCompleted, this, &BatchFullLoadProcessor::onWorkerFileCompleted, Qt::QueuedConnection);
        connect(worker, &BatchFullLoadWorker::logMessage, this, &BatchFullLoadProcessor::onWorkerLog, Qt::QueuedConnection);
        connect(worker, &BatchFullLoadWorker::bucketFinished, this, &BatchFullLoadProcessor::onBucketFinished, Qt::QueuedConnection);
        connect(worker, &BatchFullLoadWorker::bucketFinished, thread, &QThread::quit);
        connect(thread, &QThread::finished, this, &BatchFullLoadProcessor::onWorkerFinished, Qt::QueuedConnection);
        connect(worker, &BatchFullLoadWorker::fileMetricsReported, this, &BatchFullLoadProcessor::onFileMetricsReported, Qt::QueuedConnection);
...
void BatchFullLoadProcessor::runReaderLoop(QList<FileInfo> files){
...
void BatchFullLoadProcessor::runWriterLoop() {
    while (true) {
        BatchWriteTask task;
        bool hasTask = false;
        {
...
void BatchFullLoadProcessor::runHybridLoop(QList<FileInfo> files) {
    int fileIndex = 0;
...
QList<FileInfo> BatchFullLoadProcessor::scanAndProbeFiles(const QString& rootPath, bool includeSubfolders){
    ...
    while (it.hasNext()) {
        ...
        if (scanTimer.elapsed() >= 10000) {
            emit logMessage(BatchFullLoadUtils::getCurrentTimestamp() + tr(" [正在扫描] 已经扫描到 %1 个有效音频文件").arg(validFiles.size()));
            scanTimer.restart();
        }
    }
    return validFiles;
...
std::optional<FileInfo> BatchFullLoadProcessor::probeFile(const QString& path, bool& outIsVideo){
    outIsVideo = false;
    AVFormatContext* format_ctx_raw = nullptr;
    if (avformat_open_input(&format_ctx_raw, path.toUtf8().constData(), nullptr, nullptr) != 0) {
        return std::nullopt;
    }
    BatchFullLoadFFmpegRAII::BatchFullLoadFormatContextPtr formatContext(format_ctx_raw);
    ...
    if (codec->id == AV_CODEC_ID_AAC) {
        BatchFullLoadFFmpegRAII::BatchFullLoadPacketPtr pkt = BatchFullLoadFFmpegRAII::make_packet();
        int64_t maxPts = 0;
...
void BatchFullLoadProcessor::pause() { m_pauseFlag = true; emit logMessage(BatchFullLoadUtils::getCurrentTimestamp() + tr(" 任务已暂停")); emit batchPaused(); }

void BatchFullLoadProcessor::resume() { m_pauseFlag = false; emit logMessage(BatchFullLoadUtils::getCurrentTimestamp() + tr(" 任务已恢复")); emit batchResumed(); }

void BatchFullLoadProcessor::stop() { 
    m_stopFlag = true; 
    m_ioCV.notify_all();
    m_writeCV.notify_all(); 
    if (m_pauseFlag.load()) { resume(); } 
    emit logMessage(BatchFullLoadUtils::getCurrentTimestamp() + tr(" 正在终止任务 请稍候")); 
}

void BatchFullLoadProcessor::onAsyncWriteCompleted(const QString& filePath, bool success, const QString& errorInfo, const FilePerformanceMetrics& metrics, int threadId) {
    onFileMetricsReported(metrics);
    onWorkerFileCompleted(filePath, success, errorInfo, metrics, threadId);
}

void BatchFullLoadProcessor::onWorkerFileCompleted(const QString& filePath, bool success, const QString& errorInfo, const FilePerformanceMetrics& metrics, int threadId) {
    ...
    QString timestamp = BatchFullLoadUtils::getCurrentTimestamp();
    if (success) {
...
void BatchFullLoadProcessor::onWorkerLog(const QString& message) { emit logMessage(message); }

void BatchFullLoadProcessor::onBucketFinished(int bucketId, qint64 elapsedMs) { std::lock_guard<std::mutex> lock(m_bucketDataMutex); m_bucketCompletionTimes[bucketId] = elapsedMs; }

void BatchFullLoadProcessor::onFileMetricsReported(const FilePerformanceMetrics& metrics) {
    std::lock_guard<std::mutex> lock(m_metricsMutex);
    m_allFileMetrics.append(metrics);
}

void BatchFullLoadProcessor::checkAndFinishBatch() {
    ...
    if (!m_isCleaningUp.exchange(true)) {
        ...
        QString totalTimeStr = BatchFullLoadUtils::formatElapsedMs(elapsedMs);
        QString timestamp = BatchFullLoadUtils::getCurrentTimestamp();
        emit logMessage("----------------------------------------");
        {
            ...
            while (i.hasNext()) {
                i.next();
                if (i.value() > 0) {
                    QString timeStr = BatchFullLoadUtils::formatElapsedMs(i.value());
        ...
        if (wasStoppedByUser) {
            summary = tr("任务已终止");
            emit logMessage(BatchFullLoadUtils::getCurrentTimestamp() + tr(" 任务结束"));
...
void BatchFullLoadProcessor::onWorkerFinished() {
...
void BatchFullLoadProcessor::onIoThreadFinished() { 
    checkAndFinishBatch(); 
}

void BatchFullLoadProcessor::onWriterThreadFinished() { 
    checkAndFinishBatch(); 
}

void BatchFullLoadProcessor::cleanup() {
    m_stopFlag = true; 
    m_ioCV.notify_all();
    m_writeCV.notify_all();
    ...
    m_bufferPool = QQueue<BatchTask>();
    m_currentPoolBytes = 0;
    m_writeQueue = QQueue<BatchWriteTask>();
    m_currentWritePoolBytes = 0;
