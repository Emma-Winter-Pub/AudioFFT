#include "BatchFullLoadImageExporter.h"
...
BatchFullLoadImageExporter::BatchFullLoadImageExporter() {}

BatchFullLoadImageExporter::~BatchFullLoadImageExporter() {}

ExportStatus BatchFullLoadImageExporter::exportImage(
    ...
    double maxDb)
{
    ...
    if (!settings.enableComponents && !settings.enableGrid) {
        std::unique_ptr<XImageEncoder> encoder = ImageEncoderFactory::createEncoder(settings.exportFormat);
        if (!encoder) return ExportStatus::Failure;
        bool success = encoder->EncodeAndSave(imageToProcess, outputFilePath, settings.qualityLevel);
    ...
    if (audioDuration > 0) {
        ...
        if (settings.enableComponents) {
            QString totalTimeLabel = BatchFullLoadUtils::formatPreciseDuration(audioDuration);
...
int BatchFullLoadImageExporter::calculateBestFreqStep(double maxFreqKhz, int availableHeight) const {
...
int BatchFullLoadImageExporter::calculateBestDbStep(double dbRange, int availableHeight) const {
