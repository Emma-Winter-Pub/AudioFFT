...
#include "XSpectrumDataProvider.h"
...
public:
    ...
    virtual void setPalette(const QString& paletteId, bool inverted, bool negative);
    ...
    void setSpectrumProfileStyle(bool visible, const QColor& color, int lineWidth, bool filled, int alpha, SpectrumProfileType type, SpectrumProfileDirection direction);
    ...
    void setExternalDataProvider(XSpectrumDataProvider* provider);
    void setDataConfig(DataSourceType spectrumSource, DataSourceType probeSource, int probePrecision);
    virtual void tryAutoExpand(bool isExternalOpen);
...
protected:
    ...
    XSpectrumDataProvider* m_spectrumProvider = nullptr;
    ImageSpectrumDataProvider* m_imageProvider = nullptr;
    XSpectrumDataProvider* m_externalProvider = nullptr;
...
private slots:
    void onAutoExpandValueChanged(const QVariant& value);
    void onAutoExpandFinished();

private:
    void updateOverlayDataUnderMouse(bool forceUpdate = false);
    QVariantAnimation* m_expandAnimation = nullptr;
    bool m_isAutoExpanding = false;
};