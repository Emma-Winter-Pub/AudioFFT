...
bool BatchStreamDecoder::open(std::shared_ptr<BatchStreamBuffer> buffer, const QString& filePathHint, BatchStreamAudioInfo& info, int codecIdHint) {
    close();
    if (!buffer) return false;
    int ioBufferSize = 65536;
    if (codecIdHint == AV_CODEC_ID_APE) {
        ioBufferSize = 4 * 1024 * 1024;
    }
...
bool BatchStreamDecoder::setupDecoder(const QString& filePath, BatchStreamAudioInfo& info) {
    ...
    if (isSeekable && (codec->id == AV_CODEC_ID_AAC || codec->id == AV_CODEC_ID_MP3)) {
        ...
        if (av_seek_frame(m_formatCtx.get(), m_audioStreamIndex, hugeTimestamp, AVSEEK_FLAG_BACKWARD) >= 0) {
            BatchStreamFFmpegRAII::PacketPtr tempPkt(av_packet_alloc());
            int64_t maxPts = 0;
            bool readAny = false;
