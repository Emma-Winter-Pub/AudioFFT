      ...
      m_externalFileToLoad(externalFile)
{
    ...
    connect(m_batWidget, &BatWidget::pauseBatchRequested, m_processor, &BatProcessor::pause, Qt::DirectConnection);
    connect(m_batWidget, &BatWidget::resumeBatchRequested, m_processor, &BatProcessor::resume, Qt::DirectConnection);
    connect(m_batWidget, &BatWidget::stopBatchRequested, m_processor, &BatProcessor::stop, Qt::DirectConnection);
    ...
    connect(m_batWidget, &BatWidget::pauseBatchStreamRequested, m_batchStreamProcessor, &BatchStreamProcessor::pause, Qt::DirectConnection);
    connect(m_batWidget, &BatWidget::resumeBatchStreamRequested, m_batchStreamProcessor, &BatchStreamProcessor::resume, Qt::DirectConnection);
    connect(m_batWidget, &BatWidget::stopBatchStreamRequested, m_batchStreamProcessor, &BatchStreamProcessor::stop, Qt::DirectConnection);
...
void MainWindow::processExternalFile(const QString& filePath) {
    if (filePath.isEmpty()) return;
    if (m_currentWorkspaceIndex == 2) {
        if (m_batWidget && m_batWidget->isBusy()) {
            return;
        } else {
            GlobalPreferences prefs = GlobalPreferences::load();
    ...
    GlobalPreferences prefs = GlobalPreferences::load();
    if (m_currentWorkspaceIndex == 0 && m_fastStreamingWidget) {
        m_fastStreamingWidget->loadFile(filePath);
    } else if (m_currentWorkspaceIndex == 1 && m_singleFileWidget) {
        m_singleFileWidget->loadFile(filePath);
    }
