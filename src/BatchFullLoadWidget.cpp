...
BatchFullLoadWidget::BatchFullLoadWidget(QWidget *parent)
    : QWidget(parent)
{
    m_currentSettings = BatchSettings(); 
    setupUi();
    retranslateUi();
    setControlsEnabledForBatch(false); 
}

BatchFullLoadWidget::~BatchFullLoadWidget(){}

void BatchFullLoadWidget::setPalette(const QString& id, bool inverted, bool negative){ 
    m_currentSettings.paletteId = id; 
    m_currentSettings.paletteInverted = inverted; 
    m_currentSettings.paletteNegative = negative;
}

void BatchFullLoadWidget::setupUi(){
    ...
    m_btnInput->setFocusPolicy(Qt::NoFocus);
    ...
    m_btnOutput->setFocusPolicy(Qt::NoFocus);
    ...
    layout->addSpacing(2);
    m_logView = new LogListView(this);
    ...
    m_logView->setStyleSheet(
        "QListView { background-color: rgb(78, 90, 110); color: rgb(217, 217, 217); border: 1px solid #2B333E; outline: none; }"
        "QListView::item { padding: 1px 2px; }"
        "QListView::item:selected { background-color: #4A90E2; color: white; }"
    );
    m_logModel->appendLog(tr("[提示] 批量处理已准备就绪 （点击设置进行配置）"));
    layout->addWidget(m_logView);
    connect(m_btnInput, &QPushButton::clicked, this, &BatchFullLoadWidget::onBrowseInputPath);
    connect(m_btnOutput, &QPushButton::clicked, this, &BatchFullLoadWidget::onBrowseOutputPath);
    connect(m_btnSettings, &QPushButton::clicked, this, &BatchFullLoadWidget::onSettingsClicked);
    connect(m_btnStart, &QPushButton::clicked, this, &BatchFullLoadWidget::onStartClicked);
    connect(m_btnStop, &QPushButton::clicked, this, &BatchFullLoadWidget::onStopClicked);
}

void BatchFullLoadWidget::retranslateUi() {
...
void BatchFullLoadWidget::updateButtonSizes(){
...
void BatchFullLoadWidget::changeEvent(QEvent *event) {
...
void BatchFullLoadWidget::onBrowseInputPath(){
...
void BatchFullLoadWidget::onBrowseOutputPath(){
...
void BatchFullLoadWidget::onSettingsClicked(){
    BatchFullLoadWidgetSettings dlg(m_currentSettings, this);
    if (dlg.exec() == QDialog::Accepted) {
        ...
        appendLog("--------------------------------------------------");
        appendLog(BatchFullLoadUtils::getCurrentTimestamp() + tr(" 设置参数已更新"));
        bool isFullLoad = (m_currentSettings.mode == BatchMode::FullLoad);
        appendLog(BatchFullLoadUtils::formatSettings(m_currentSettings, isFullLoad));
    }
}

BatchStreamSettings BatchFullLoadWidget::convertToStreamSettings(const BatchSettings& s) const{
    ...
    bs.paletteId = s.paletteId;
    bs.paletteInverted = s.paletteInverted;
    bs.paletteNegative = s.paletteNegative;
    bs.minDb = s.minDb;
    bs.maxDb = s.maxDb;
    ...
    bs.enableWhitelist = s.enableWhitelist;
    bs.whitelistExtensions = s.whitelistExtensions;
    bs.excludeVideoFiles = s.excludeVideoFiles;
    bs.categorizeByCodec = s.categorizeByCodec;
    return bs;
}

void BatchFullLoadWidget::onStartClicked(){
...
void BatchFullLoadWidget::onScanFinished(QSharedPointer<FileSnapshot> snapshotB){
    ...
    if (shouldPrompt) {
        ...
        if (reply == QMessageBox::No) {
            appendLog(BatchFullLoadUtils::getCurrentTimestamp() + tr(" 任务已经取消"));
            onBatchStopped(); 
            return;
        }
    }
...
void BatchFullLoadWidget::onStopClicked(){
...
void BatchFullLoadWidget::appendLog(const QString& message){
    if (message.isEmpty()) m_logModel->clear();
    else m_logModel->appendLog(message);
}

void BatchFullLoadWidget::updateProgress(int current, int total){
    Q_UNUSED(current);
    Q_UNUSED(total);
}

void BatchFullLoadWidget::setControlsEnabledForBatch(bool isRunning){
...
void BatchFullLoadWidget::onBatchStarted(){
    m_currentState = BatchState::Running;
    m_btnStart->setText(tr("暂停任务"));
    setControlsEnabledForBatch(true);
}

void BatchFullLoadWidget::onBatchPaused(){
    m_currentState = BatchState::Paused;
    m_btnStart->setText(tr("继续任务"));
    appendLog(tr("任务已暂停"));
}

void BatchFullLoadWidget::onBatchResumed(){
    m_currentState = BatchState::Running;
    m_btnStart->setText(tr("暂停任务"));
    appendLog(tr("任务已恢复"));
}

void BatchFullLoadWidget::onBatchStopped(){
...
void BatchFullLoadWidget::onBatchFinished(const QString& summaryReport){
    m_currentState = BatchState::Idle;
    m_btnStart->setText(tr("开始任务"));
    setControlsEnabledForBatch(false);
    appendLog(tr("==================== 任务完成 ===================="));
