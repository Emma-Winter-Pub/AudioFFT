...
void BatchStreamWorker::startWorkLoop(const BatchStreamSettings& settings) {
    ...
    while (true) {
        ...
        qint64 elapsedMs = 0;
        try {
            success = processSingleFile(task, settings, fft, generator, errorMsg, isResized, elapsedMs);
        ...
        if (m_directWriteMode || !success) {
            ...
            emit fileFinished(filePath, success, success ? successMsg : errorMsg, m_bucketId, elapsedMs);
        }
    }
...
bool BatchStreamWorker::processSingleFile(BatchStreamTask& task, const BatchStreamSettings& settings, 
                                          ...
                                          bool& outIsResized, qint64& elapsedMsOut) 
{
    outIsResized = false;
    QString filePath = task.fileInfo.path;
    ...
    if (task.buffer) {
        openSuccess = decoder.open(task.buffer, filePath, info, task.fileInfo.codecId);
    } else {
    ...
    if (!openSuccess) {
        ...
        elapsedMsOut = fileTimer.elapsed();
        return false;
    }
    ...
    generator.init(settings.imageHeight, fftSize, info.sampleRate, settings.curveType, settings.paletteId, settings.paletteInverted, settings.paletteNegative, settings.minDb, settings.maxDb);
    ...
    bool wasIndexed = (validSpectrogram.format() == QImage::Format_Indexed8);
    QList<QRgb> colorTable = wasIndexed ? validSpectrogram.colorTable() : QList<QRgb>();
    ...
    if (settings.exportFormat == "JPG") {
        maxFormatDim = BatchFullLoadConfig::JPEG_MAX_DIMENSION;
    } else if (settings.exportFormat == "WebP") {
        maxFormatDim = BatchFullLoadConfig::BAT_WEBP_MAX_DIMENSION;
    } else if (settings.exportFormat == "AVIF") {
        maxFormatDim = BatchFullLoadConfig::BAT_AVIF_MAX_DIMENSION;
    }
    int topMargin = 0; int bottomMargin = 0; int leftMargin = 0; int rightMargin = 0;
    if (settings.enableComponents) {
        topMargin = AppConfig::IMAGE_SAVE_TOP_MARGIN;
        bottomMargin = AppConfig::IMAGE_SAVE_BOTTOM_MARGIN;
        leftMargin = AppConfig::IMAGE_SAVE_LEFT_MARGIN;
        rightMargin = AppConfig::IMAGE_SAVE_RIGHT_MARGIN;
    }
    ...
    if (wasIndexed && validSpectrogram.format() != QImage::Format_Indexed8) {
        validSpectrogram = validSpectrogram.convertToFormat(QImage::Format_Indexed8, colorTable);
    }
    ...
    QString outPath = task.fileInfo.outputFilePath;
    ...
    if (m_directWriteMode) {
        ...
        writeTask.elapsedMs = elapsedMsOut;
