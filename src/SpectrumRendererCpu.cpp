...
void SpectrumRendererCpu::paintEvent(QPaintEvent* event) {
    ...
    if (dataCount > static_cast<size_t>(drawLen * 2)) {
        ...
        for (int i = 0; i < drawLen; ++i) {
            ...
            downsampledData[i] = maxVal;
        }
        pDisplayData = &downsampledData;
    }
    size_t count = pDisplayData->size();
    if (count < 1) return;
    ...
    if (m_type == SpectrumProfileType::Bar) {
        ...
        QPainterPath path;
        if (m_direction == SpectrumProfileDirection::Horizontal) {
            path.moveTo(r.left(), r.bottom() - calcAmplitude((*pDisplayData)[0], h));
            double xStep = static_cast<double>(w) / (count - 1);
            for (size_t i = 1; i < count; ++i) {
                double x = r.left() + i * xStep;
                double y = r.bottom() - calcAmplitude((*pDisplayData)[i], h);
                path.lineTo(x, y);
            }
            if (m_isFilled) {
            ...
            path.moveTo(r.left() + calcAmplitude((*pDisplayData)[0], w), r.bottom());
            double yStep = static_cast<double>(h) / (count - 1);
            for (size_t i = 1; i < count; ++i) {
                double y = r.bottom() - i * yStep;
                double x = r.left() + calcAmplitude((*pDisplayData)[i], w);
                path.lineTo(x, y);
            }
            if (m_isFilled) {
                QPainterPath fillPath = path;
