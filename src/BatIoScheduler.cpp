...
BatExecutionPlan BatIoScheduler::analyze(const QString& inputPath, const QString& outputPath, int threadCount) {
    BatExecutionPlan plan;
    ...
    if (inIsSSD && outIsSSD) {
        ...
        plan.ioMode = BatIoThreadMode::WriterOnly;
        plan.workerDirectWrite = false;
        plan.strategyName = QObject::tr("[检测] 输出路径为低速储存器");
    }
    else if (!inIsSSD && outIsSSD) {
        plan.ioMode = BatIoThreadMode::ReaderOnly;
        plan.workerDirectWrite = true;
