...
void SingleFileWidget::onTrackActionTriggered(int index) {
    if (m_isCueMode) {
        ...
        if (m_isFullCacheAvailable && !isFileChanged) {
            ...
            m_currentPcmData = slicePcmData(m_fullCachedPcmData, m_currentMetadata.sampleRate, track.getStartSeconds(), track.getEndSeconds());
            emit startReProcessing(
                m_currentPcmData, m_currentMetadata, intervalToSend, m_currentHeight,
                m_currentFftSize, m_currentCurveType, m_currentMinDb, m_currentMaxDb,
            ...
            emit startProcessing(m_currentlyProcessedFile,
                ...
                m_currentPaletteType, m_currentWindowType,
                -1, -1,
                track.getStartSeconds(), track.getEndSeconds(),
                ProcessMode::Full);
        }
        return;
    }
...
void SingleFileWidget::onSaveClicked() {
    ...
    PaletteType paletteType = m_currentPaletteType;
    auto future = QtConcurrent::run([imageToSave, fName, durationSec, showGrid, preciseDurationStr, sampleRate, quality, outputFilePath, fmt, curveType, minDb, maxDb, paletteType, drawComponents]() {
        return ImageExporter::exportImage(
            imageToSave, fName, durationSec, showGrid, preciseDurationStr, sampleRate,
            quality, outputFilePath, fmt, curveType, minDb, maxDb, paletteType,
            drawComponents
        );
    });
...
void SingleFileWidget::loadFile(const QString& filePath) {
    onFileSelected(filePath);
}
...
void SingleFileWidget::updateSpectrumProfileStyle(bool visible, const QColor& color, int lineWidth, bool filled, int alpha, SpectrumProfileType type, SpectrumProfileDirection direction) {
    if (m_spectrogramViewer) {
