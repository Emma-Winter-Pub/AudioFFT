...
BatchExecutionPlan BatchFullLoadIoScheduler::analyze(const QString& inputPath, const QString& outputPath, int threadCount) {
    ...
    if (inIsSSD && outIsSSD) {
        ...
        plan.ioMode = BatchIoThreadMode::WriterOnly;
        plan.workerDirectWrite = false;
        plan.strategyName = QObject::tr("[检测] 输出路径为低速储存器");
    }
    else if (!inIsSSD && outIsSSD) {
        plan.ioMode = BatchIoThreadMode::ReaderOnly;
        plan.workerDirectWrite = true;
