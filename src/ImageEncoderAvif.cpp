...
bool ImageEncoderAvif::EncodeAndSave(const QImage& image, const QString& filePath, int quality) const {
    if (image.width() > AppConfig::IMAGE_AVIF_MAX_DIMENSION || image.height() > AppConfig::IMAGE_AVIF_MAX_DIMENSION) {
        return false;
    }
...
QByteArray ImageEncoderAvif::EncodeToMemory(const QImage& image, int quality) const {
    if (image.width() > AppConfig::IMAGE_AVIF_MAX_DIMENSION || image.height() > AppConfig::IMAGE_AVIF_MAX_DIMENSION) {
        return QByteArray();
    }
    QImage imageToSave = image.convertToFormat(QImage::Format_RGBA8888);
