...
#include "StreamingSpectrogramGenerator.h"
...
StreamingSpectrogramProcessor::StreamingSpectrogramProcessor(QObject* parent):QThread(parent){}

StreamingSpectrogramProcessor::~StreamingSpectrogramProcessor() {stopProcessing();wait();}

void StreamingSpectrogramProcessor::startProcessing(const QString& filePath,
                                               double timeInterval, int imageHeight, int fftSize,
                                               CurveType curveType, double minDb, double maxDb,
                                               const QString& paletteId, bool paletteInverted,
                                               bool paletteNegative, FFTWindowType windowType,
                                               ...
                                               std::optional<CueSheet> cueSheet)
{
    ...
    m_paletteInverted = paletteInverted;
    m_paletteNegative = paletteNegative;
...
void StreamingSpectrogramProcessor::stopProcessing(){m_stopRequested = true;}

template <typename FftEngine, typename PcmType>
double StreamingSpectrogramProcessor::processLoop(FftEngine& fftEngine, class StreamingAudioDecoder& decoder) {
    StreamingSpectrogramGenerator generator;
    connect(&generator, &StreamingSpectrogramGenerator::logMessage, this, &StreamingSpectrogramProcessor::logMessage, Qt::DirectConnection);
    ...
    int hopSize = std::max(1, static_cast<int>(std::round(sampleRate * effectiveInterval)));
    if (!fftEngine.configure(m_fftSize, hopSize, m_windowType, 0.0)) {
        emit processingFailed(tr("        错误：傅里叶变换初始化失败 内存不足或参数错误"));
        return 0.0;
    }
    emit logMessage(StreamingUtils::getCurrentTimestamp() + " " + tr("开始傅里叶变换"));
    emit logMessage(tr("        [FFT点数] %1").arg(m_fftSize));
    emit logMessage(tr("        [窗函数] %1").arg(FFTWindowFunctions::getName(m_windowType)));
    emit logMessage(tr("        [时间精度] %1").arg(precisionStr));
    emit logMessage(tr("        [步进]  %1 采样点").arg(hopSize));
    if constexpr (std::is_same_v<PcmType, StreamingTypes::StreamingPcmData64>) {
        emit logMessage(tr("        [计算精度] 64 位浮点数"));
    } else {
        emit logMessage(tr("        [计算精度] 32 位浮点数"));
    }
    emit logMessage(StreamingUtils::getCurrentTimestamp() + " " + tr("开始图像渲染"));
    emit logMessage(tr("        [映射] %1").arg(MappingCurves::getName(m_curveType)));
    QString palName = m_paletteId;
    auto availPals = ColorPaletteFactory::instance().getAvailablePalettes();
    for(const auto& p : availPals) { if(p.first == m_paletteId) { palName = QString("%1 %2").arg(p.first, p.second); break; } }
    QString palFlags;
    if (m_paletteInverted) palFlags += tr("(反向)");
    if (m_paletteNegative) palFlags += tr("(反相)");
    emit logMessage(tr("        [配色] %1 %2").arg(palName).arg(palFlags));
    emit logMessage(tr("        [ dB ] %1 ~ %2").arg(m_minDb).arg(m_maxDb));
    emit logMessage(StreamingUtils::getCurrentTimestamp() + " " + tr("循环处理文件"));
    ...
    QFuture<StreamingTypes::StreamingPcmDataVariant> futurePcm = QtConcurrent::run([&decoder, chunkSizeSamples]() {
    ...
    while (!m_stopRequested) {
        ...
        if (!spectrumData.empty()) {
            QImage imageChunk = generator.generateChunk(
                ...
                m_curveType, m_minDb, m_maxDb, m_paletteId, m_paletteInverted, m_paletteNegative
            );
...
void StreamingSpectrogramProcessor::run(){
    StreamingAudioDecoder decoder;
    connect(&decoder, &StreamingAudioDecoder::logMessage, this, &StreamingSpectrogramProcessor::logMessage, Qt::DirectConnection);
    ...
    if (decoder.isDoublePrecision()) {
        StreamingFFT64 fftEngine;
        pcmDuration = processLoop<StreamingFFT64, StreamingTypes::StreamingPcmData64>(fftEngine, decoder);
    } else {
        StreamingFFT32 fftEngine;
        pcmDuration = processLoop<StreamingFFT32, StreamingTypes::StreamingPcmData32>(fftEngine, decoder);
    }
    if (!m_stopRequested) {
        ...
        if (metadata.durationSec > 0.001) {
            if (pcmDuration > metadata.durationSec + 0.002) {
                ...
                emit logMessage(tr("        [校准] 检测到填充数据 时长更新为 %1").arg(StreamingUtils::formatPreciseDuration(usec)));
        ...
        emit logMessage(StreamingUtils::getCurrentTimestamp() + " " + tr("流式处理完成"));
        emit processingFinished(finalDuration);
    } else {
        emit logMessage(StreamingUtils::getCurrentTimestamp() + " " + tr("任务终止"));
    }
}