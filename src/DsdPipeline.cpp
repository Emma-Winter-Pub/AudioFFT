...
namespace {
    ...
    void decode_worker_generic(
        ...
        try { threadPcm.reserve(131072); } catch (...) {}
    ...
    void decode_worker_memory_wrapper(
        ...
        if (!formatCtxRaw) { has_error = true; error_msg = DsdPipeline::tr("无法从内存打开流"); return; }
        ...
        if (avformat_find_stream_info(formatCtx.get(), nullptr) < 0) { has_error = true; error_msg = DsdPipeline::tr("无法读取流信息"); return; }
    ...
    void decode_worker_file_wrapper(
        ...
        if (avformat_open_input(&formatCtxRaw, filePath.toUtf8().constData(), nullptr, nullptr) != 0) {
            has_error = true; error_msg = DsdPipeline::tr("无法打开文件"); return;
        }
...
bool DsdPipeline::execute(
    ...
    if (logCb) logCb(QString(DsdPipeline::tr("        [线程] %1 线程")).arg(num_threads));
    ...
    if (mode == ExecutionMode::LoadToRam) {
        if (logCb) logCb(getCurrentTimestamp() + DsdPipeline::tr(" 开始加载文件"));
        fileBuffer = FFmpegMemLoader::loadFileToMemory(filePath);
        if (!fileBuffer) {
            if (logCb) logCb(DsdPipeline::tr("        文件加载失败 回退到单线程模式"));
            return false;
        }
    }
    if (logCb) logCb(getCurrentTimestamp() + DsdPipeline::tr(" 开始并行解码"));
    ...
    auto process_template = [&](auto dummy_type) -> bool {
        ...
        if (has_error) {
            if (logCb) logCb(DsdPipeline::tr("        并行解码错误 ") + error_msg + DsdPipeline::tr(" 回退到单线程模式"));
            return false;
        }
