...
bool ImageEncoderTiff::EncodeAndSave(const QImage& image, const QString& filePath, int quality) const{
    (void)quality;
...
#if defined(_WIN32)
    TIFF* tif = TIFFOpenW(filePath.toStdWString().c_str(), "w");
#else
...
QByteArray ImageEncoderTiff::EncodeToMemory(const QImage& image, int quality) const {
    (void)quality;
    QImage imageToSave = image.convertToFormat(QImage::Format_RGBA8888);
    if (imageToSave.isNull()) {
        return QByteArray();
