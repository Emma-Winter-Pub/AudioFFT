...
#include "AudioDecoderAPE.h"
#include "AudioDecoderDSD.h"
...
using namespace AudioDecoderTypes;

struct PipelineRegistrar {
    PipelineRegistrar() {
        AudioDecoderFactory::instance().registerPipeline("ape", []() { return std::make_unique<AudioDecoderAPE>(); });
        AudioDecoderFactory::instance().registerPipeline("flac", []() { return std::make_unique<AudioDecoderFLAC>(); });
        AudioDecoderFactory::instance().registerPipeline("alac", []() { return std::make_unique<AudioDecoderFLAC>(); });
        auto dsdCreator = []() { return std::make_unique<AudioDecoderDSD>(); };
        AudioDecoderFactory::instance().registerPipeline("dsd_lsbf", dsdCreator);
        AudioDecoderFactory::instance().registerPipeline("dsd_msbf", dsdCreator);
        AudioDecoderFactory::instance().registerPipeline("dsd_lsbf_planar", dsdCreator);
        AudioDecoderFactory::instance().registerPipeline("dsd_msbf_planar", dsdCreator);
    }
};
...
static QString formatDuration(double total_seconds) {
    ...
    if (hours > 0) {
        return FullLoadAudioDecoder::tr("%1:%2:%3")
            .arg(hours)
            .arg(minutes, 2, 10, QChar('0'))
            .arg(seconds, 2, 10, QChar('0'));
    } else {
        return FullLoadAudioDecoder::tr("%1:%2")
            .arg(minutes)
            .arg(seconds, 2, 10, QChar('0'));
    }
}

FullLoadAudioDecoder::FullLoadAudioDecoder(QObject* parent) : QObject(parent) {}

QString FullLoadAudioDecoder::getChannelLayoutString(const AVChannelLayout* ch_layout){
    char buf[128];
    av_channel_layout_describe(ch_layout, buf, sizeof(buf));
    return QString::fromUtf8(buf);
}

void FullLoadAudioDecoder::logFileDetails(AVFormatContext* formatContext){
    ...
    if (m_tempCueSheet.has_value()) {
        ...
        for (const auto& t : sheet.tracks) {
            QString startStr = QTime::fromMSecsSinceStartOfDay(static_cast<int>(t.getStartSeconds() * 1000)).toString("HH:mm:ss");
            QString endStr;
            double endSec = t.getEndSeconds();
    ...
    auto log_tag = [this, &tag](const char* key, const QString& display_name, AVFormatContext* ctx) {
        tag = av_dict_get(ctx->metadata, key, nullptr, 0);
        if (!tag && qstrcmp(key, "track") == 0) {
            tag = av_dict_get(ctx->metadata, "tracknumber", nullptr, 0);
        }
        if (tag && strlen(tag->value) > 0) {
            QString val = TextEncodingHelper::decode(QByteArray(tag->value));
            if (qstrcmp(key, "track") == 0) {
                if (val.contains('/')) {
                    QStringList parts = val.split('/');
                    QString trackNum = parts.value(0).trimmed();
                    QString totalNum = parts.value(1).trimmed();
                    if (!trackNum.isEmpty()) {
                        emit logMessage(tr("        [%1] %2").arg(display_name).arg(trackNum));
                    }
                    if (!totalNum.isEmpty()) {
                        emit logMessage(tr("        [总计轨道] %1").arg(totalNum));
                    }
                } else {
                    emit logMessage(tr("        [%1] %2").arg(display_name).arg(val));
                    AVDictionaryEntry* tagTotal = av_dict_get(ctx->metadata, "tracktotal", nullptr, 0);
                    if (!tagTotal) tagTotal = av_dict_get(ctx->metadata, "totaltracks", nullptr, 0);
                    if (tagTotal && strlen(tagTotal->value) > 0) {
                        emit logMessage(tr("        [总计轨道] %1").arg(QString::fromUtf8(tagTotal->value)));
                    }
                }
            } else {
                emit logMessage(tr("        [%1] %2").arg(display_name).arg(val));
    ...
    log_tag("track", tr("轨道"), formatContext);
    log_tag("date", tr("日期"), formatContext);
}

bool FullLoadAudioDecoder::buildChannelMatrix(SwrContext* swr, int inputChannels, int targetChannelIdx){
...
bool FullLoadAudioDecoder::decode(const QString& filePath, int targetTrackIdx, int targetChannelIdx, double startSec, double endSec, std::optional<CueSheet> cueSheet) {
    m_pcmData = PCMData32();
    m_metadata = AudioDecoderTypes::AudioMetadata();
    ...
    for (unsigned int i = 0; i < formatCtx->nb_streams; ++i) {
        ...
        if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
            ...
            AudioDecoderTypes::AudioTrack track;
            track.index = i;
            ...
            QString lang = tagLang ? TextEncodingHelper::decode(QByteArray(tagLang->value)) : "";
    ...
    if (codec->id == AV_CODEC_ID_AAC || codec->id == AV_CODEC_ID_MP3) {
        ...
        if (av_seek_frame(formatCtx.get(), finalTrackIdx, hugeTimestamp, AVSEEK_FLAG_BACKWARD) >= 0) {
            AudioDecoderTypes::AVPacketPtr tempPkt(av_packet_alloc());
            int64_t maxPts = 0;
            bool readAny = false;
    ...
    AVDictionaryEntry* tagTitle = av_dict_get(formatCtx->metadata, "title", nullptr, 0);
    if (tagTitle && strlen(tagTitle->value) > 0) {
        m_metadata.title = TextEncodingHelper::decode(QByteArray(tagTitle->value));
    }
    AVDictionaryEntry* tagArtist = av_dict_get(formatCtx->metadata, "artist", nullptr, 0);
    if (tagArtist && strlen(tagArtist->value) > 0) {
        m_metadata.artist = TextEncodingHelper::decode(QByteArray(tagArtist->value));
    }
    ...
    auto pipeline = AudioDecoderFactory::instance().create(shortCodecName);
    bool success = false;
    if (pipeline) {
        ...
        if (isSmallEnough && isShortEnough) {
            ...
            VolumeTopology topo = StorageTopologyAnalyzer::analyze(filePath);
            
            if (topo.hasSSD && !topo.hasHDD && !topo.isNetworkStorage) {
                emit logMessage(tr("        [策略] 检测到高速设备 (%1) 使用多线程")
                                .arg(topo.finalInference.reasonCode == InferenceReasonCode::NVMeWithTrimAndNoSeekPenalty ? "NVMe" : "SSD"));
                ...
                QString reason = topo.isNetworkStorage ? tr("网络驱动器") : tr("机械/低速设备");
                emit logMessage(tr("        [策略] 检测到 %1 使用单线程").arg(reason));
                success = false;
            }
        }
    }
    if (!success) {
        ...
        success = singleThreadedDecode(filePath, finalTrackIdx, targetChannelIdx, stdStart, stdEnd);
    }
...
bool FullLoadAudioDecoder::singleThreadedDecode(const QString& filePath, int trackIndex, int targetChannelIdx, double startSec, double endSec) {
    AVFormatContext* formatCtxRaw = nullptr;
