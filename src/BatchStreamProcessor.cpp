...
void BatchStreamProcessor::scanDirectory(const BatchStreamSettings& settings) {
    ...
    m_totalTimer.start();
    emit batchStarted();
    emit logMessage(BatchStreamUtils::getCurrentTimestamp() + tr(" 准备开始任务"));
    emit logMessage(BatchFullLoadUtils::formatSettings(settings, false));
    emit logMessage("--------------------------------------------------");
    ...
    m_planFuture = QtConcurrent::run([=]() {
        return BatchStreamIoScheduler::analyze(settings.inputPath, settings.outputPath, totalThreadQuota);
    });
    ...
    emit logMessage(BatchStreamUtils::getCurrentTimestamp() + QString(tr(" [扫描完毕] 共找到 %1 个有效音频文件")).arg(validFiles.size()));
...
QList<BatchStreamScannedFile> BatchStreamProcessor::performScan(const QString& rootPath, bool includeSubfolders){
    ...
    while (it.hasNext()) {
        ...
        if (m_currentSettings.enableWhitelist) {
            ...
            auto infoOpt = probeFile(path, isVideo);
            if (infoOpt.has_value()) {
                validFiles.append(infoOpt.value());
            } else {
        ...
        if (scanTimer.elapsed() >= 10000) {
            emit logMessage(BatchStreamUtils::getCurrentTimestamp() + tr(" [正在扫描] 已经扫描到 %1 个有效音频文件").arg(validFiles.size()));
            scanTimer.restart();
        }
    }
    return validFiles;
...
std::optional<BatchStreamScannedFile> BatchStreamProcessor::probeFile(const QString& path, bool& outIsVideo){
    outIsVideo = false;
    ...
    BatchStreamFFmpegRAII::FormatContextPtr formatContext(format_ctx_raw);
    ...
    if (codec->id == AV_CODEC_ID_AAC) {
        BatchStreamFFmpegRAII::PacketPtr pkt(av_packet_alloc());
        int64_t maxPts = 0;
        bool readAny = false;
    ...
    if (!durationFound) {
        if (stream->duration != AV_NOPTS_VALUE) {
            durationMs = static_cast<qint64>(stream->duration * av_q2d(stream->time_base) * 1000);
...
void BatchStreamProcessor::startWorkers(QList<BatchStreamScannedFile>& allFiles) {
    ...
    BatchStreamExecutionPlan plan = m_planFuture.result();
    ...
    QString inputPathNorm = QDir::cleanPath(QFileInfo(m_currentSettings.inputPath).absoluteFilePath());
    QDir inputDir(inputPathNorm);
    QDir outputDir(m_currentSettings.outputPath);
    QString suffix;
    const QString& fmt = m_currentSettings.exportFormat;
    ...
    for (auto& file : allFiles) {
        ...
        if (m_currentSettings.categorizeByCodec && !file.codecName.isEmpty()) {
            subPath = QDir(subPath).filePath(file.codecName.toUpper());
            subPath = QDir(subPath).filePath(rootDirName);
        }
        if (m_currentSettings.reuseSubfolderStructure) {
...
void BatchStreamProcessor::runHybridLoop(QList<BatchStreamScannedFile> files, int concurrency) {
    ...
    while (!m_stopFlag.load()) {
        ...
        bool isQueueEmpty = false;
        {
            ...
            if (!isQueueEmpty) {
                if (m_currentWriteBytes > WRITE_BATCH_TRIGGER || isReadPhaseFinished || m_workersDone.load()) {
                    shouldWrite = true;
                }
            }
        }
        if (m_workersDone.load() && isQueueEmpty && isReadPhaseFinished) {
            break;
        }
        if (shouldWrite) {
            while (!m_stopFlag.load()) {
...
void BatchStreamProcessor::onAsyncWriteCompleted(const QString& filePath, bool success, const QString& errorInfo, qint64 elapsedMs, int threadId) {
    onWorkerFileFinished(filePath, success, errorInfo, threadId, elapsedMs);
}
...
void BatchStreamProcessor::checkAndFinishBatch() {
    if (m_activeThreadCount.load() > 0) return;
    bool ioThreadRunning = (m_ioThread && m_ioThread->isRunning());
    bool writerThreadRunning = (m_writerThread && m_writerThread->isRunning());
    if (ioThreadRunning || writerThreadRunning) return;
...
void BatchStreamProcessor::cleanupThreads() {
    ...
    m_invalidWhiteListFiles = QStringList();
    m_unexpectedValidFiles = QStringList();
