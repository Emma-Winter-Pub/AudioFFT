...
int FullLoadSpectrogramPainter::calculateBestFreqStepPng(double maxFreqKhz, int availableHeight) {
...
int FullLoadSpectrogramPainter::calculateBestDbStep(double dbRange, int availableHeight) {
...
FullLoadSpectrogramPainter::FullLoadSpectrogramPainter(QObject *parent) : QObject(parent) {}

QImage FullLoadSpectrogramPainter::drawFinalImage(
    ...
    bool drawComponents)
{
    if (!drawComponents && !showGrid) {
        return spectrogramToSave;
    }
    ...
    if (useMultiThreading) {
        return createFinalImage_MultiThread(
            spectrogramToSave, fileName, audioDuration, showGrid, preciseDurationStr, nativeSampleRate, curveType, minDb, maxDb, paletteId, paletteInverted, paletteNegative, drawComponents
        );
    } else {
...
QImage FullLoadSpectrogramPainter::createFinalImage_MultiThread(
    ...
    bool drawComponents)
{
    ...
    const int topMargin = drawComponents ? AppConfig::IMAGE_SAVE_TOP_MARGIN : 0;
    const int bottomMargin = drawComponents ? AppConfig::IMAGE_SAVE_BOTTOM_MARGIN : 0;
    const int leftMargin = drawComponents ? AppConfig::IMAGE_SAVE_LEFT_MARGIN : 0;
    const int rightMargin = drawComponents ? AppConfig::IMAGE_SAVE_RIGHT_MARGIN : 0;
    const int colorBarWidth = drawComponents ? AppConfig::IMAGE_SAVE_COLOR_BAR_WIDTH : 0;
    ...
    if (drawComponents) {
        ...
        const auto& colorTable = ColorPaletteFactory::instance().getPalette(paletteId, paletteInverted, paletteNegative);
...
QImage FullLoadSpectrogramPainter::createFinalImage_SingleThread(
    const QImage& rawSpectrogram,
    const QString& fileName,
    double audioDuration,
    bool showGrid,
