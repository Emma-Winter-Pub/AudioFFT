#include "StreamingAudioDecoder.h"
...
static QString formatDuration(double total_seconds) {
    ...
    if (hours > 0) {
        return StreamingAudioDecoder::tr("%1:%2:%3")
            .arg(hours)
            .arg(minutes, 2, 10, QChar('0'))
            .arg(seconds, 2, 10, QChar('0'));
    } else {
        return StreamingAudioDecoder::tr("%1:%2")
            .arg(minutes)
            .arg(seconds, 2, 10, QChar('0'));
    }
}

StreamingAudioDecoder::StreamingAudioDecoder(QObject* parent):QObject(parent){}

StreamingAudioDecoder::~StreamingAudioDecoder() {close();}

void StreamingAudioDecoder::resetState() {
    m_formatCtx.reset();
    m_codecCtx.reset();
    m_swrCtx.reset();
    m_packet.reset();
    m_frame.reset();
    m_metadata = StreamingTypes::StreamingAudioMetadata();
    m_streamIndex = -1;
...
void StreamingAudioDecoder::close() {
    std::lock_guard<std::recursive_mutex> lock(m_mutex);
    resetState();
}

QString StreamingAudioDecoder::getChannelLayoutString(const AVChannelLayout* ch_layout) {
    char buf[128];
    av_channel_layout_describe(ch_layout, buf, sizeof(buf));
    return QString::fromUtf8(buf);
}

void StreamingAudioDecoder::logFileDetails(AVFormatContext* formatContext, double totalDuration) {
    emit logMessage(tr("    文件信息："));
    emit logMessage(tr("        [路径] %1").arg(m_metadata.filePath));
    emit logMessage(tr("        [大小] %1").arg(StreamingUtils::formatSize(QFileInfo(m_metadata.filePath).size())));
...
bool StreamingAudioDecoder::buildChannelMatrix(SwrContext* swr, int inputChannels, int targetChannelIdx) {
...
bool StreamingAudioDecoder::open(const QString& filePath, int targetTrackIdx, int targetChannelIdx,
    ...
    std::optional<CueSheet> cueSheet)
{
    ...
    emit logMessage(StreamingUtils::getCurrentTimestamp() + " " + tr("探测文件"));
    ...
    m_formatCtx = StreamingTypes::AVFormatContextPtr(formatCtxRaw);
    ...
    for (unsigned int i = 0; i < m_formatCtx->nb_streams; ++i) {
        ...
        if (st->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
            ...
            StreamingTypes::FastAudioTrack track;
            track.index = i;
    ...
    m_codecCtx = StreamingTypes::AVCodecContextPtr(avcodec_alloc_context3(codec));
    ...
    if (codec->id == AV_CODEC_ID_AAC || codec->id == AV_CODEC_ID_MP3) {
        ...
        if (av_seek_frame(m_formatCtx.get(), m_streamIndex, hugeTimestamp, AVSEEK_FLAG_BACKWARD) >= 0) {
            StreamingTypes::AVPacketPtr tempPkt(av_packet_alloc());
            int64_t maxPts = 0;
            bool readAny = false;
    ...
    AVDictionaryEntry* tagTitle = av_dict_get(m_formatCtx->metadata, "title", nullptr, 0);
    if (tagTitle && strlen(tagTitle->value) > 0) {
        m_metadata.title = TextEncodingHelper::decode(QByteArray(tagTitle->value));
    }
    AVDictionaryEntry* tagArtist = av_dict_get(m_formatCtx->metadata, "artist", nullptr, 0);
    ...
    m_swrCtx = StreamingTypes::SwrContextPtr(swrCtxRaw);
    ...
    m_packet = StreamingTypes::AVPacketPtr(av_packet_alloc());
    m_frame = StreamingTypes::AVFramePtr(av_frame_alloc());
    emit logMessage(StreamingUtils::getCurrentTimestamp() + " " + tr("开始音频解码"));
...
StreamingTypes::StreamingPcmDataVariant StreamingAudioDecoder::readNextChunk(size_t minSamples) {
    std::lock_guard<std::recursive_mutex> lock(m_mutex);
    StreamingTypes::StreamingPcmData32 pcm32;
    StreamingTypes::StreamingPcmData64 pcm64;
...
bool StreamingAudioDecoder::seek(double timestampSeconds) {
    std::lock_guard<std::recursive_mutex> lock(m_mutex);
