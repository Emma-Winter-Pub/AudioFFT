...
namespace {
    ...
    protected:
        void paintEvent(QPaintEvent* event) override {
            ...
            if (idx >= 0) {
                ...
                if (!pm.isNull()) {
                    QPainter p(this);
                    QRect iconRect(rect().right() - 25 - 180, rect().top() + (rect().height() - 14) / 2, 180, 14);
    ...
    public:
        ...
        void paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const override {
            ...
            QRect textRect = opt.rect.adjusted(5, 0, -180, 0);
            painter->drawText(textRect, Qt::AlignLeft | Qt::AlignVCenter, text);
            if (!pm.isNull()) {
                QRect iconRect(opt.rect.right() - 190, opt.rect.top() + (opt.rect.height() - 14) / 2, 180, 14);
        ...
        QSize sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const override {
            ...
            s.setWidth(fm.horizontalAdvance(text) + 180 + 30);
            return s;
        }
    };
}
...
void GlobalSettingsDialogUI::setupUi(QDialog* parent) {
    ...
    int row1 = 0;
    m_cbMultiInstance = new QCheckBox(QObject::tr("启用 (允许多个 AudioFFT 进程同时运行)"), parent);
    addRow(layoutTab1, row1, QObject::tr("* 多实例："), m_cbMultiInstance);
    m_cbAutoPlayExternal = new QCheckBox(QObject::tr("启用 (关联 AudioFFT 打开时将自动播放)"), parent);
    addRow(layoutTab1, row1, QObject::tr("* 关联打开播放："), m_cbAutoPlayExternal);
    m_cmbDefaultWorkspace = new QComboBox(parent);
    m_cmbDefaultWorkspace->addItem(QObject::tr("流式"), 0);
    m_cmbDefaultWorkspace->addItem(QObject::tr("全量"), 1);
    addRow(layoutTab1, row1, QObject::tr("* 默认工作空间："), m_cmbDefaultWorkspace);
    ...
    m_spinWidthVal->setFixedWidth(80);
    ...
    QWidget* paletteContainer = new QWidget(parent);
    QHBoxLayout* paletteLayout = new QHBoxLayout(paletteContainer);
    paletteLayout->setContentsMargins(0, 0, 0, 0);
    paletteLayout->setSpacing(6);
    m_cmbPalette = new PaletteComboBox(parent);
    m_cmbPalette->setIconSize(QSize(180, 14));
    auto createPaletteIcon = [](const QString& id) -> QIcon {
        QPixmap pixmap(180, 14);
        QPainter p(&pixmap);
    ...
    for (const auto& p : palettes) {
        m_cmbPalette->addItem(QString("%1    %2").arg(p.first).arg(p.second), p.first);
        m_cmbPalette->setItemData(m_cmbPalette->count() - 1, createPaletteIcon(p.first).pixmap(QSize(180, 14)), Qt::UserRole + 1);
    }
    m_cmbPalette->setItemDelegate(new PaletteComboBoxDelegate(m_cmbPalette));
    m_cbPaletteInvert = new QCheckBox(QObject::tr("颜色反向"), parent);
    m_cbPaletteNegative = new QCheckBox(QObject::tr("颜色反相"), parent);
    paletteLayout->addWidget(m_cbPaletteInvert, 0);
    paletteLayout->addWidget(m_cbPaletteNegative, 0);
    paletteLayout->addWidget(m_cmbPalette, 1);
    addRow(layoutTab3, row3, QObject::tr("* 配色："), paletteContainer);
    ...
    m_cmbProfileDirection = new QComboBox(parent);
    m_cmbProfileDirection->addItem(QObject::tr("水平"), QVariant::fromValue(SpectrumProfileDirection::Horizontal));
    m_cmbProfileDirection->addItem(QObject::tr("竖直"), QVariant::fromValue(SpectrumProfileDirection::Vertical));
    addRow(layoutTab4, row4, QObject::tr("方向："), m_cmbProfileDirection);
    ...
    profColorLayout->addWidget(profPresetContainer);
    ...
    colorLayout->addWidget(presetContainer);
    ...
    int row7 = 0;
    m_cbAutoExpandOnPlay = new QCheckBox(QObject::tr("启用 (从起点播放时自动展开频谱图)"), parent);
    addRow(layoutTab7, row7, QObject::tr("自动展开："), m_cbAutoExpandOnPlay);
    ...
    handleColorLayout->addSpacing(30);
    ...
    handleColorLayout->addWidget(handlePresetContainer);
    ...
    phColorLayout->addWidget(phPresetContainer);
