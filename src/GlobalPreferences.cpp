...
GlobalPreferences GlobalPreferences::load() {
    ...
    prefs.windowType = static_cast<FFTWindowType>(settings.value("windowType", static_cast<int>(prefs.windowType)).toInt());
    prefs.curveType = static_cast<CurveType>(settings.value("curveType", static_cast<int>(prefs.curveType)).toInt());
    prefs.paletteId = settings.value("paletteId", prefs.paletteId).toString();
    if (prefs.paletteId.isEmpty()) {
        prefs.paletteId = "0000";
    }
    prefs.paletteInverted = settings.value("paletteInverted", prefs.paletteInverted).toBool();
    prefs.paletteNegative = settings.value("paletteNegative", prefs.paletteNegative).toBool();
    ...
    // 播放器
    prefs.autoExpandOnPlay = settings.value("autoExpandOnPlay", prefs.autoExpandOnPlay).toBool();
...
void GlobalPreferences::save(const GlobalPreferences& prefs) {
    ...
    settings.setValue("paletteId", prefs.paletteId);
    settings.setValue("paletteInverted", prefs.paletteInverted);
    settings.setValue("paletteNegative", prefs.paletteNegative);
    ...
    // 播放器
    settings.setValue("autoExpandOnPlay", prefs.autoExpandOnPlay);
