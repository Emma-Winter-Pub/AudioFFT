...
ScreenshotSaveDialog::ScreenshotSaveDialog(
    ...
    : QDialog(parent)
{
    ...
    QString defaultBaseName = QString("AudioFFT_Screenshot_%1").arg(timestamp);
...
void ScreenshotSaveDialog::setupUi(bool isJpegAvailable, bool isWebpAvailable, bool isAvifAvailable) {
    setStyleSheet(R"(
        QDialog { background-color: #3B4453; color: #E0E0E0; }
        QLabel { font-size: 12px; color: #E0E0E0; }
        QLineEdit { background-color: rgb(78, 90, 110); border: 1px solid #2B333E; color: #E0E0E0; padding: 2px; font-size: 11px; }
        QComboBox { font-size: 12px; height: 18px; background-color: rgb(78, 90, 110); border: 1px solid #2B333E; color: #E0E0E0; padding-left: 2px; }
        QComboBox QAbstractItemView { background-color: rgb(78, 90, 110); selection-background-color: #5A687A; color: #E0E0E0; border: 1px solid #2B333E; outline: none; }
        QPushButton { font-size: 12px; min-width: 60px; min-height: 20px; padding: 0 10px; background-color: rgb(78, 90, 110); color: #E0E0E0; border: 1px solid #1C222B; }
    ...
    connect(m_browseButton, &QPushButton::clicked, this, &ScreenshotSaveDialog::onBrowsePath);
    ...
    if (isJpegAvailable) m_formatComboBox->addItem("JPG  ( *.jpg )", "JPG");
    m_formatComboBox->addItem("JPEG 2000  ( *.jp2 )", "JPEG 2000");
    if (isWebpAvailable) m_formatComboBox->addItem("WebP  ( *.webp )", "WebP");
    if (isAvifAvailable) m_formatComboBox->addItem("AVIF  ( *.avif )", "AVIF");
    ...
    connect(m_buttonBox, &QDialogButtonBox::accepted, this, &ScreenshotSaveDialog::onAccept);
    connect(m_buttonBox, &QDialogButtonBox::rejected, this, &QDialog::reject);
    connect(m_formatComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged), this, &ScreenshotSaveDialog::onFormatChanged);
...
void ScreenshotSaveDialog::onAccept() {
...
void ScreenshotSaveDialog::onBrowsePath() {
...
QString ScreenshotSaveDialog::getSelectedFormatIdentifier() const {
    return m_formatComboBox->currentData().toString();
}

QString ScreenshotSaveDialog::getSelectedFilePath() const {
...
int ScreenshotSaveDialog::qualityLevel() const {
