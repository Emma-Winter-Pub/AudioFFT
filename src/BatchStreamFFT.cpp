...
bool BatchStreamFFT::init(int fftSize, double overlapPercent, FFTWindowType windowType, bool precision64) {
    ...
    if (m_isDouble) {
        ...
        m_windowDouble = FFTWindowFunctions::generate(N, windowType);
    } else {
        ...
        std::vector<double> w = FFTWindowFunctions::generate(N, windowType);
...
BatchStreamSpectrum32 BatchStreamFFT::process32(const BatchStreamPcm32& newData) {
    ...
    while (offset + N <= m_residue32.size()) {
        ...
        for (int i = 0; i < binCount; ++i) {
            ...
            resPtr[i] = 10.0f * std::log10(power + 1e-18f);
        }
        offset += m_hopSize;
    }
...
BatchStreamSpectrum64 BatchStreamFFT::process64(const BatchStreamPcm64& newData) {
    ...
    while (offset + N <= m_residue64.size()) {
        ...
        for (int i = 0; i < binCount; ++i) {
            ...
            resPtr[i] = 10.0 * std::log10(power + 1e-18);
        }
        offset += m_hopSize;
    }
