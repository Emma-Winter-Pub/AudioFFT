    ...
    : QWidget(parent)
{
    ...
    m_expandAnimation->setDuration(1500);
    m_expandAnimation->setEasingCurve(QEasingCurve::InOutCubic);
    connect(m_expandAnimation, &QVariantAnimation::valueChanged, this, &SpectrogramViewerBase::onAutoExpandValueChanged);
    connect(m_expandAnimation, &QVariantAnimation::finished, this, &SpectrogramViewerBase::onAutoExpandFinished);
}
...
void SpectrogramViewerBase::setExternalDataProvider(XSpectrumDataProvider* provider) {
...
void SpectrogramViewerBase::tryAutoExpand(bool isExternalOpen) {
    if (!m_autoExpandOnPlay) return;
    if (isExternalOpen) return;
    if (m_playheadTime > 0.001) return;
    if (m_isViewLocked) return;
    int contentW = getContentWidth();
    if (contentW <= 1) return;
    QRect viewRect = getSpectrogramRect();
    double fitZoomX = static_cast<double>(viewRect.width()) / contentW;
    if (fitZoomX >= 1.0) return;
    if (std::abs(m_zoomX - fitZoomX) > 0.0001) {
        return;
    }
    abortAutoExpand();
    m_isAutoExpanding = true;
    m_expandAnimation->setStartValue(m_zoomX);
    m_expandAnimation->setEndValue(1.0);
    m_expandAnimation->start();
}

void SpectrogramViewerBase::abortAutoExpand() {
    if (m_isAutoExpanding) {
        m_expandAnimation->stop();
        m_isAutoExpanding = false;
    }
}

void SpectrogramViewerBase::onAutoExpandValueChanged(const QVariant& value) {
    m_zoomX = value.toDouble();
    m_offsetX = 0.0;
    ...
    if (m_crosshairOverlay) {
        ...
        bool effectiveVisible = m_isPlayheadVisible && (pps > 0);
...
void SpectrogramViewerBase::onAutoExpandFinished() {
    m_isAutoExpanding = false;
    m_zoomX = 1.0;
    m_offsetX = 0.0;
    clampViewPosition();
...
void SpectrogramViewerBase::setSpectrumProfileStyle(bool visible, const QColor& color, int lineWidth, bool filled, int alpha, SpectrumProfileType type, SpectrumProfileDirection direction) {
    ...
    if (m_spectrumOverlay) {
        if (!visible) {
            ...
            m_spectrumOverlay->setStyle(color, lineWidth, filled, alpha, type, direction);
        }
    }
}
...
void SpectrogramViewerBase::setAutoExpandOnPlay(bool enabled) {
    m_autoExpandOnPlay = enabled;
}
...
void SpectrogramViewerBase::setPlayheadPosition(double seconds) {
    ...
    if (m_isAutoExpanding && std::abs(seconds - m_playheadTime) > 0.5) {
        abortAutoExpand();
    }
    double pps = getPixelsPerSecond();
    QRect viewRect = getSpectrogramRect();
...
void SpectrogramViewerBase::updateSpectrumOverlayFromTime(double timeSec) {
    XSpectrumDataProvider* activeProvider = m_imageProvider;
...
void SpectrogramViewerBase::setPalette(const QString& paletteId, bool inverted, bool negative) {
    m_currentPaletteId = paletteId;
    m_paletteInverted = inverted;
    m_paletteNegative = negative;
    update();
}
...
void SpectrogramViewerBase::drawOverlay(QPainter& painter) {
    ...
    if (m_showComponents) {
        ...
        const auto& colorTable = ColorPaletteFactory::instance().getPalette(m_currentPaletteId, m_paletteInverted, m_paletteNegative);
        int steps = colorTable.size();
        for (int i = 0; i < steps; ++i) {
            gradient.setColorAt(1.0 - (double)i / (steps - 1), QColor(colorTable[i]));
        }
    ...
    if (m_isMouseInView) {
        ...
        if (m_probeSource == DataSourceType::FftRawData && m_externalProvider) {
            ...
            if (relativeY >= 0 && relativeY < params.imageHeight && 
                ...
                double mouseFreqHz = ratio * maxFreq;
