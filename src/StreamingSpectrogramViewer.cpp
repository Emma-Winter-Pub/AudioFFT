...
StreamingSpectrogramViewer::StreamingSpectrogramViewer(QWidget *parent)
...
int StreamingSpectrogramViewer::getContentWidth() const {
    return m_isProcessing ? m_currentWriteX : m_canvas.width();
}

int StreamingSpectrogramViewer::getContentHeight() const {
    return m_canvas.height();
}

void StreamingSpectrogramViewer::reset() {
    abortAutoExpand();
...
void StreamingSpectrogramViewer::initCanvas(int estimatedWidth, int height) {
    abortAutoExpand();
...
void StreamingSpectrogramViewer::appendChunk(const QImage& chunk) {
...
int StreamingSpectrogramViewer::getColorIndex(const QPoint& viewPos) const {
...
void StreamingSpectrogramViewer::setFinished() {
    m_isProcessing = false;
    m_isViewLocked = false;
...
void StreamingSpectrogramViewer::setPalette(const QString& paletteId, bool inverted, bool negative) {
    SpectrogramViewerBase::setPalette(paletteId, inverted, negative);
    updateColorTable();
}

int StreamingSpectrogramViewer::getReferenceWidth() const {
    return m_canvas.width();
}

void StreamingSpectrogramViewer::updateColorTable() {
    if (m_canvas.format() != QImage::Format_Indexed8) return;
    m_canvas.setColorTable(ColorPaletteFactory::instance().getPalette(m_currentPaletteId, m_paletteInverted, m_paletteNegative));
}

void StreamingSpectrogramViewer::paintEvent(QPaintEvent* event) {
    Q_UNUSED(event);
...
void StreamingSpectrogramViewer::resizeEvent(QResizeEvent* event) {
