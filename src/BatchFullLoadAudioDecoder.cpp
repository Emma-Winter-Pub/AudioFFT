    ...
    )
{
    ...
    while (ret >= 0) {
        ...
        if (converted_samples > 0) {
            pcm_data.insert(pcm_data.end(), temp_buffer.data(), temp_buffer.data() + converted_samples);
        }
...
BatchFullLoadAudioDecoder::BatchFullLoadAudioDecoder() {}

BatchFullLoadAudioDecoder::~BatchFullLoadAudioDecoder() {}

std::optional<BatchFullLoadDecodedAudio> BatchFullLoadAudioDecoder::decode(const QString& filePath){
...
std::optional<BatchFullLoadDecodedAudio> BatchFullLoadAudioDecoder::decode(SharedFileBuffer buffer){
...
std::optional<BatchFullLoadDecodedAudio> BatchFullLoadAudioDecoder::decodeInternal(AVFormatContext* formatContext){
    ...
    auto codec_ctx = BatchFullLoadFFmpegRAII::make_codec_context(codec);
    ...
    swr_alloc_set_opts2(&swr_ctx_raw,
                        ...
                        0, nullptr);
    BatchFullLoadFFmpegRAII::BatchFullLoadSwrContextPtr swr_ctx(swr_ctx_raw);
    if (!swr_ctx || swr_init(swr_ctx.get()) < 0) {
        return std::nullopt;
    }
    auto packet = BatchFullLoadFFmpegRAII::make_packet();
    auto frame = BatchFullLoadFFmpegRAII::make_frame();
    ...
    BatchFullLoadDecodedAudio result;
