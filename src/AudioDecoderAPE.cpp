...
namespace {
    ...
    template <typename T>
    bool is_constant_anchor(const std::vector<T, AlignedAllocator<T, 64>>& data, size_t offset, size_t len) {
        if (offset + len > data.size() || len == 0) return true;
        T min_val = data[offset];
        ...
        if constexpr (std::is_floating_point_v<T>) {
            T threshold = std::numeric_limits<T>::epsilon() * static_cast<T>(4.0);
            if (range <= threshold) {
                return true;
            }
        } else {
    ...
    void decode_segment_ffmpeg(
        ...
        BatchFullLoadFFmpegRAII::BatchFullLoadCodecContextPtr codecCtx = BatchFullLoadFFmpegRAII::make_codec_context(codec);
        ...
        BatchFullLoadFFmpegRAII::BatchFullLoadSwrContextPtr swrCtx(swr_raw);
        ...
        BatchFullLoadFFmpegRAII::BatchFullLoadFramePtr frame = BatchFullLoadFFmpegRAII::make_frame();
    ...
    void worker_entry(
        ...
        if (isMemMode) {
            fmt_raw = FFmpegMemLoader::createFormatContextFromMemory(memFile, &ioState);
        } else {
            avformat_open_input(&fmt_raw, filePath.toUtf8().constData(), nullptr, nullptr);
        }
        if (!fmt_raw) { *has_error = true; return; }
        std::unique_ptr<AVFormatContext, void(*)(AVFormatContext*)> fmt_ctx(
            fmt_raw,
            isMemMode ? [](AVFormatContext* ptr){ FFmpegMemLoader::cleanup(ptr); }
                      : [](AVFormatContext* ptr){ avformat_close_input(&ptr); }
        );
        if (avformat_find_stream_info(fmt_ctx.get(), nullptr) < 0) { *has_error = true; return; }
        decode_segment_ffmpeg<T>(fmt_ctx.get(), trackIdx, chIdx, start, end, (*results)[id].raw_data, *has_error);
        if (!*has_error) {
    ...
    public:
        static bool run(
            ...
            if (total_sec < 120.0) {
                if (logCb) logCb(AudioDecoderAPE::tr("        [检查] 音频时长太短 使用单线程"));
                return false;
            }
            ...
            if (os_threads <= 1) {
                if (logCb) logCb(AudioDecoderAPE::tr("        [检查] 系统为单线程 使用单线程"));
                return false;
            }
            ...
            while (true) {
                if (logCb) logCb(AudioDecoderAPE::tr("        [执行] 使用%1线程 重试%2次 短音频偏移%3次 长音频偏移%4次").arg(n).arg(w).arg(x).arg(y));
                ...
                for (int i = 0; i < n - 1; ++i) {
                    ...
                    if (is_constant_anchor(curr.raw_data, anchor_start_idx, ANCHOR_SIZE)) {
                        j1_all_valid = false;
                        problem_cut_index = i + 1;
                        break;
                    }
                ...
                }
                {
                    ...
                    for (int i = 0; i < n - 1; ++i) {
                        ...
                        if (pos < 0) {
                            ...
                            problem_cut_index = i + 1;
                            if (pos == -2 && logCb) {
                                logCb(AudioDecoderAPE::tr("        [警告] 存在多处匹配 放弃并行解码"));
                            }
                            break;
                        }
            ...
            LABEL_U:
                ...
                if (logCb) logCb(AudioDecoderAPE::tr("        [警告] 验证失败 进入错误处理流程"));
            ...
            LABEL_X:
                ...
                if (x == 1) {
                    if (problem_cut_index != -1) {
                        ...
                        if (logCb) logCb(AudioDecoderAPE::tr("        [偏移] 分割点%1 向前移动10秒").arg(problem_cut_index));
                    }
                    continue; 
                } else {
                    return false;
            ...
            LABEL_Y:
                ...
                if (y == 1) {
                    ...
                    if (problem_cut_index != -1) {
                        shift_offsets[problem_cut_index] = 10.0; 
                        if (logCb) logCb(AudioDecoderAPE::tr("        [偏移] 分割点%1 向后移动10秒").arg(problem_cut_index));
                    }
                    continue;
                } else {
                    return false;
...
bool AudioDecoderAPE::execute(
    ...
    BatchFullLoadFFmpegRAII::BatchFullLoadFormatContextPtr fmtCtx(fmtRaw);
