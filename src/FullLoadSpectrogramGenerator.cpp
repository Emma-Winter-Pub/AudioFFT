...
FullLoadSpectrogramGenerator::FullLoadSpectrogramGenerator(QObject* parent) : QObject(parent) {}

struct RenderChunk { int start_x; int width; };

QImage FullLoadSpectrogramGenerator::generate(
    ...
    bool paletteNegative)
{
    ...
    auto computeWorker = [&](auto&& flatData) -> QImage {
        ...
        auto renderTask = [=](const RenderChunk& chunk) -> QImage {
            ...
            if (chunkImage.isNull()) return QImage();
            std::vector<uchar*> scanLines(targetHeight);
            for (int y = 0; y < targetHeight; ++y) {
                scanLines[y] = chunkImage.scanLine(y);
            }
            for (int x_local = 0; x_local < chunk.width; ++x_local) {
                int x_global = chunk.start_x + x_local;
                size_t dataBaseIndex = static_cast<size_t>(x_global) * numBins;
                for (int y_img = 0; y_img < targetHeight; ++y_img) {
                    ...
                    scanLines[y_img][x_local] = color_index;
                }
            }
            return chunkImage;
        };
        ...
        if (resultImage.isNull()) {
            emit logMessage(tr("        错误：内存不足 图像生成失败"));
            return QImage();
        }
