...
struct CurveParam {
    double mul;      // 10^k - 1
    double invLog;   // 1 / (k * LN10)
    double expCoeff; // k * LN10
    double invMul;   // 1 / (10^k - 1)
    double offset;   // 10^k
};

constexpr CurveParam D_TABLE[] = {
    {9.0,           1.0 / (1.0 * LN10),  1.0 * LN10,  1.0 / 9.0,           1e1},
    {99.0,          1.0 / (2.0 * LN10),  2.0 * LN10,  1.0 / 99.0,          1e2},
    {999.0,         1.0 / (3.0 * LN10),  3.0 * LN10,  1.0 / 999.0,         1e3},
    {9999.0,        1.0 / (4.0 * LN10),  4.0 * LN10,  1.0 / 9999.0,        1e4},
    {99999.0,       1.0 / (5.0 * LN10),  5.0 * LN10,  1.0 / 99999.0,       1e5},
    {999999.0,      1.0 / (6.0 * LN10),  6.0 * LN10,  1.0 / 999999.0,      1e6},
    {9999999.0,     1.0 / (7.0 * LN10),  7.0 * LN10,  1.0 / 9999999.0,     1e7},
    {99999999.0,    1.0 / (8.0 * LN10),  8.0 * LN10,  1.0 / 99999999.0,    1e8},
    {999999999.0,   1.0 / (9.0 * LN10),  9.0 * LN10,  1.0 / 999999999.0,   1e9},
    {9999999999.0,  1.0 / (10.0 * LN10), 10.0 * LN10, 1.0 / 9999999999.0,  1e10}
};

inline double curveD(double x, int level) {
    const auto& p = D_TABLE[level];
    return std::log1p(p.mul * x) * p.invLog;
}

inline double curveFD(double x, int level) {
    return 1.0 - curveD(1.0 - x, level);
}

inline double curveInvD(double y, int level) {
    const auto& p = D_TABLE[level];
    return (std::exp(p.expCoeff * y) - 1.0) * p.invMul; 
}

inline double curveInvFD(double y, int level) {
    const auto& p = D_TABLE[level];
    return (p.offset - std::exp(p.expCoeff * (1.0 - y))) * p.invMul;
}
...
*/
    ...
    case CurveType::D1:  return curveD(x, 0);
    case CurveType::D2:  return curveD(x, 1);
    case CurveType::D3:  return curveD(x, 2);
    case CurveType::D4:  return curveD(x, 3);
    case CurveType::D5:  return curveD(x, 4);
    case CurveType::D6:  return curveD(x, 5);
    case CurveType::D7:  return curveD(x, 6);
    case CurveType::D8:  return curveD(x, 7);
    case CurveType::D9:  return curveD(x, 8);
    case CurveType::D10: return curveD(x, 9);

    case CurveType::FD1:  return curveFD(x, 0);
    case CurveType::FD2:  return curveFD(x, 1);
    case CurveType::FD3:  return curveFD(x, 2);
    case CurveType::FD4:  return curveFD(x, 3);
    case CurveType::FD5:  return curveFD(x, 4);
    case CurveType::FD6:  return curveFD(x, 5);
    case CurveType::FD7:  return curveFD(x, 6);
    case CurveType::FD8:  return curveFD(x, 7);
    case CurveType::FD9:  return curveFD(x, 8);
    case CurveType::FD10: return curveFD(x, 9);
...
*/
    ...
    case CurveType::D1:  return curveInvD(y, 0);
    case CurveType::D2:  return curveInvD(y, 1);
    case CurveType::D3:  return curveInvD(y, 2);
    case CurveType::D4:  return curveInvD(y, 3);
    case CurveType::D5:  return curveInvD(y, 4);
    case CurveType::D6:  return curveInvD(y, 5);
    case CurveType::D7:  return curveInvD(y, 6);
    case CurveType::D8:  return curveInvD(y, 7);
    case CurveType::D9:  return curveInvD(y, 8);
    case CurveType::D10: return curveInvD(y, 9);

    case CurveType::FD1:  return curveInvFD(y, 0);
    case CurveType::FD2:  return curveInvFD(y, 1);
    case CurveType::FD3:  return curveInvFD(y, 2);
    case CurveType::FD4:  return curveInvFD(y, 3);
    case CurveType::FD5:  return curveInvFD(y, 4);
    case CurveType::FD6:  return curveInvFD(y, 5);
    case CurveType::FD7:  return curveInvFD(y, 6);
    case CurveType::FD8:  return curveInvFD(y, 7);
    case CurveType::FD9:  return curveInvFD(y, 8);
    case CurveType::FD10: return curveInvFD(y, 9);
