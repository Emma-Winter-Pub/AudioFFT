...
namespace {
    ...
    void decode_worker_generic(
        ...
        double chunk_duration = chunk.trim_end_sec - chunk.trim_start_sec;
        if (chunk_duration < 0) chunk_duration = 0;
        size_t expected_samples = static_cast<size_t>(chunk_duration * codecCtx->sample_rate * 1.1);
        try { 
            threadPcm.reserve(expected_samples); 
        } catch (...) {}
    ...
    void decode_worker_memory_wrapper(
        ...
        if (!formatCtxRaw) { has_error = true; error_msg = AudioDecoderDSD::tr("无法从内存打开流"); return; }
        ...
        if (avformat_find_stream_info(formatCtx.get(), nullptr) < 0) { has_error = true; error_msg = AudioDecoderDSD::tr("无法读取流信息"); return; }
    ...
    void decode_worker_file_wrapper(
        ...
        if (avformat_open_input(&formatCtxRaw, filePath.toUtf8().constData(), nullptr, nullptr) != 0) {
            has_error = true; error_msg = AudioDecoderDSD::tr("无法打开文件"); return;
        }
        BatchFullLoadFFmpegRAII::BatchFullLoadFormatContextPtr formatCtx(formatCtxRaw);
        if (avformat_find_stream_info(formatCtx.get(), nullptr) < 0) { has_error = true; error_msg = AudioDecoderDSD::tr("无法读取流信息"); return; }
        decode_worker_generic<T>(formatCtx.get(), trackIndex, channelIndex, chunk, all_results, results_mutex, has_error, error_msg);
    }
}

bool AudioDecoderDSD::execute(
    ...
    if (logCb) logCb(QString(AudioDecoderDSD::tr("        [线程] %1 线程")).arg(num_threads));
    ...
    if (mode == ExecutionMode::LoadToRam) {
        if (logCb) logCb(getCurrentTimestamp() + AudioDecoderDSD::tr(" 开始加载文件"));
        fileBuffer = FFmpegMemLoader::loadFileToMemory(filePath);
        if (!fileBuffer) {
            if (logCb) logCb(AudioDecoderDSD::tr("        文件加载失败 回退到单线程模式"));
            return false;
        }
    }
    if (logCb) logCb(getCurrentTimestamp() + AudioDecoderDSD::tr(" 开始并行解码"));
    ...
    auto process_template = [&](auto dummy_type) -> bool {
        ...
        if (has_error) {
            if (logCb) logCb(AudioDecoderDSD::tr("        并行解码错误 ") + error_msg + AudioDecoderDSD::tr(" 回退到单线程模式"));
            return false;
        }
        ...
        for(auto& pair : results) {
            ...
            AlignedVector<T>().swap(pair.second); 
        }
