...
public:
    ...
    const std::vector<float>& process(const void* srcData, int srcNbSamples);
    const std::vector<float>& process(const uint8_t** srcData, int srcNbSamples);
    void reset();
...
private:
    ...
    const std::vector<float>& processInternal(const uint8_t** inputData, int srcNbSamples);
};