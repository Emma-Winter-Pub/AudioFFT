...
namespace {
    ...
    OPJ_SIZE_T opj_file_write(void* p_buffer, OPJ_SIZE_T p_nb_bytes, void* p_user_data) {
        FILE* fp = static_cast<FILE*>(p_user_data);
        size_t written = fwrite(p_buffer, 1, p_nb_bytes, fp);
        return written == p_nb_bytes ? static_cast<OPJ_SIZE_T>(written) : static_cast<OPJ_SIZE_T>(-1);
    }
    OPJ_OFF_T opj_file_skip(OPJ_OFF_T p_nb_bytes, void* p_user_data) {
        FILE* fp = static_cast<FILE*>(p_user_data);

#if defined(_WIN32)
        if (_fseeki64(fp, p_nb_bytes, SEEK_CUR) != 0) return static_cast<OPJ_OFF_T>(-1);
#elif defined(__linux__) || defined(__APPLE__)
        if (fseeko(fp, p_nb_bytes, SEEK_CUR) != 0) return static_cast<OPJ_OFF_T>(-1);
#else
        if (fseek(fp, static_cast<long>(p_nb_bytes), SEEK_CUR) != 0) return static_cast<OPJ_OFF_T>(-1);
#endif

        return p_nb_bytes;
    }
    OPJ_BOOL opj_file_seek(OPJ_OFF_T p_nb_bytes, void* p_user_data) {
        FILE* fp = static_cast<FILE*>(p_user_data);

#if defined(_WIN32)
        if (_fseeki64(fp, p_nb_bytes, SEEK_SET) != 0) return OPJ_FALSE;
#elif defined(__linux__) || defined(__APPLE__)
        if (fseeko(fp, p_nb_bytes, SEEK_SET) != 0) return OPJ_FALSE;
#else
        if (fseek(fp, static_cast<long>(p_nb_bytes), SEEK_SET) != 0) return OPJ_FALSE;
#endif
...
bool ImageEncoderJpeg2000::EncodeAndSave(const QImage& image, const QString& filePath, int quality) const {
    ...
    opj_stream_set_user_data(stream, fp, nullptr);
    opj_stream_set_write_function(stream, opj_file_write);
    opj_stream_set_skip_function(stream, opj_file_skip);
    opj_stream_set_seek_function(stream, opj_file_seek);
...
QByteArray ImageEncoderJpeg2000::EncodeToMemory(const QImage& image, int quality) const {
    QImage imageToSave = image.convertToFormat(QImage::Format_RGB888);
