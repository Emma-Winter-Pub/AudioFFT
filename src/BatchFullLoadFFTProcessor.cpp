...
BatchFullLoadFFTProcessor::BatchFullLoadFFTProcessor(QObject *parent) : QObject(parent) {}

BatchFullLoadFFTProcessor::~BatchFullLoadFFTProcessor() {
...
std::optional<SpectrumDataVariant> BatchFullLoadFFTProcessor::compute(
...
std::optional<SpectrumData32> BatchFullLoadFFTProcessor::computeFloat(
    const PCMData32& pcmData, double timeInterval, int fftSize, int sampleRate, FFTWindowType windowType)
{
    ...
    if (!m_ctxFloat.isValid(fftSize, windowType)) {
        ...
        std::vector<double> tempWin = FFTWindowFunctions::generate(N, windowType);
    ...
    if (needPadding) {
        ...
        float normFactorSq = static_cast<float>(N) * static_cast<float>(N);
        for (int i = 0; i < binCount; ++i) {
            float re = outBase[i][0];
            float im = outBase[i][1];
            float power = (re * re + im * im) / normFactorSq;
            if (i > 0 && i < binCount - 1) power *= 4.0f;
            dstPtr[outOffset + i] = 10.0f * std::log10(power + 1e-18f);
        }
...
std::optional<SpectrumData64> BatchFullLoadFFTProcessor::computeDouble(
    const PCMData64& pcmData, double timeInterval, int fftSize, int sampleRate, FFTWindowType windowType)
{
    if (pcmData.empty() || fftSize <= 0 || sampleRate <= 0) return std::nullopt;
    if (!m_ctxDouble.isValid(fftSize, windowType)) {
        ...
        m_ctxDouble.windowData = FFTWindowFunctions::generate(N, windowType);
    }
    const int N = fftSize;
    ...
    while (currentFrameIdx < fullFrames) {
        ...
        for (int b = 0; b < framesToProcess; ++b) {
            ...
            for (int i = 0; i < binCount; ++i) {
                double re = frameOut[i][0];
                double im = frameOut[i][1];
    ...
    if (needPadding) {
        ...
        double normFactorSq = static_cast<double>(N) * static_cast<double>(N);
        for (int i = 0; i < binCount; ++i) {
            double re = outBase[i][0];
            double im = outBase[i][1];
            double power = (re * re + im * im) / normFactorSq;
            if (i > 0 && i < binCount - 1) power *= 4.0;
            dstPtr[outOffset + i] = 10.0 * std::log10(power + 1e-18);
        }
