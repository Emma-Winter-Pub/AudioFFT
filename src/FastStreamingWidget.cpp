...
void FastStreamingWidget::onSaveClicked() {
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        QFuture<bool> future = QtConcurrent::run([imageToSave, defaultBaseName, durationSec, grid, preciseDurationStr, sampleRate, curveType, minDb, maxDb, paletteType, drawComponents, fmt, outPath, quality]() {
...
void FastStreamingWidget::loadFile(const QString& filePath) {
...
void FastStreamingWidget::updateSpectrumProfileStyle(bool visible, const QColor& color, int lineWidth, bool filled, int alpha, SpectrumProfileType type, SpectrumProfileDirection direction) {
    if (m_viewer) {
