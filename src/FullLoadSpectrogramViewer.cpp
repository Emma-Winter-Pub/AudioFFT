...
FullLoadSpectrogramViewer::FullLoadSpectrogramViewer(QWidget* parent)
...
int FullLoadSpectrogramViewer::getContentWidth() const {
    return m_spectrogramImage.width();
}

int FullLoadSpectrogramViewer::getContentHeight() const {
    return m_spectrogramImage.height();
}

int FullLoadSpectrogramViewer::getColorIndex(const QPoint& viewPos) const {
...
void FullLoadSpectrogramViewer::setSpectrogramImage(const QImage& image, bool resetView) {
    abortAutoExpand();
...
void FullLoadSpectrogramViewer::setLoadingState(bool loading) {
    m_isLoading = loading;
    m_isViewLocked = loading;
    update();
}

void FullLoadSpectrogramViewer::paintEvent(QPaintEvent* event) {
    Q_UNUSED(event);
