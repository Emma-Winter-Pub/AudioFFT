...
void ScreenshotManager::triggerScreenshot(CaptureMode mode) {
    ...
    if (target) {
        ...
        QMetaObject::invokeMethod(this, [this, safeTarget, mode](){
            if (safeTarget) {
                performCapture(safeTarget.data(), mode);
            }
        }, Qt::QueuedConnection);
    }
}
...
void ScreenshotManager::performCapture(QWidget* target, CaptureMode mode) {
    ...
    for (QWidget* w : QApplication::topLevelWidgets()) {
        if (w == target || !w->isVisible()) continue;
        bool isChildOfTarget = false;
        QObject* parentObj = w->parent();
        while (parentObj) {
            if (parentObj == target) {
        ...
        if (isChildOfTarget) {
            QPoint targetGlobal = target->mapToGlobal(QPoint(0, 0));
            QPoint wGlobal = w->mapToGlobal(QPoint(0, 0));
            QPoint offset = wGlobal - targetGlobal;
            p.drawPixmap(offset, w->grab());
        }
    }
    p.end();
