...
FFTW3_FP64_nT_nF::FFTW3_FP64_nT_nF() : m_impl(std::make_unique<Impl>()) {}

FFTW3_FP64_nT_nF::~FFTW3_FP64_nT_nF() = default;

bool FFTW3_FP64_nT_nF::initialize(const FFTParameters& params) {
    m_params = params;
...
std::optional<SpectrumDataVariant> FFTW3_FP64_nT_nF::compute(const PCMDataVariant& inputPcm) {
    ...
    if (needPadding) {
        ...
        for (int i = 0; i < m_impl->binCount; ++i) {
            ...
            double power = (re * re + im * im) / normFactorSq;
            if (i > 0 && i < m_impl->binCount - 1) power *= 4.0;
            dstPtr[i] = 10.0 * std::log10(power + 1e-18);
        }
    }
