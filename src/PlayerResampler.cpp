...
const std::vector<float>& PlayerResampler::processInternal(const uint8_t** inputData, int srcNbSamples) {
    if (!m_swrCtx) {
        m_outBuffer.clear();
        return m_outBuffer;
    }
    ...
    int requiredSize = outNbSamplesPerChannel * m_targetChannels + 32;
    if (m_outBuffer.capacity() < requiredSize) {
        m_outBuffer.reserve(requiredSize);
    }
    m_outBuffer.resize(requiredSize);
    uint8_t* outPtr = reinterpret_cast<uint8_t*>(m_outBuffer.data());
    ...
    if (convertedSamplesPerChannel > 0) {
        m_outBuffer.resize(convertedSamplesPerChannel * m_targetChannels);
    } else {
        m_outBuffer.clear();
    }
    return m_outBuffer;
...
const std::vector<float>& PlayerResampler::process(const void* srcData, int srcNbSamples) {
...
const std::vector<float>& PlayerResampler::process(const uint8_t** srcData, int srcNbSamples) {
