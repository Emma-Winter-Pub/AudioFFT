...
void GlobalSettingsDialog::setupConnections() {
    ...
    connect(ui->m_sliderProfileFps, &QSlider::valueChanged, this, [this](int val) {
        if (val >= ui->m_sliderPlayerFps->value()) {
            ui->m_sliderPlayerFps->setValue(val);
        }
    });
    connect(ui->m_sliderPlayerFps, &QSlider::valueChanged, this, [this](int val) {
        if (val <= ui->m_sliderProfileFps->value()) {
            ui->m_sliderProfileFps->setValue(val);
        }
    });
...
void GlobalSettingsDialog::updateUiFromPrefs() {
    ...
    // 常规
    ui->m_cbMultiInstance->setChecked(m_currentPrefs.allowMultipleInstances);
    ui->m_cbAutoPlayExternal->setChecked(m_currentPrefs.autoPlayOnExternalOpen);
    int wsIdx = ui->m_cmbDefaultWorkspace->findData(m_currentPrefs.defaultWorkspace);
    if (wsIdx >= 0) ui->m_cmbDefaultWorkspace->setCurrentIndex(wsIdx);
    ...
    idx = ui->m_cmbPalette->findData(m_currentPrefs.paletteId);
    if (idx >= 0) ui->m_cmbPalette->setCurrentIndex(idx);
    ui->m_cbPaletteInvert->setChecked(m_currentPrefs.paletteInverted);
    ui->m_cbPaletteNegative->setChecked(m_currentPrefs.paletteNegative);
    ...
    onProfileTypeChanged(ui->m_cmbProfileType->currentIndex()); // trigger UI
    int dirIdx = ui->m_cmbProfileDirection->findData(QVariant::fromValue(m_currentPrefs.spectrumProfileDirection));
    if (dirIdx >= 0) ui->m_cmbProfileDirection->setCurrentIndex(dirIdx);
    ...
    // 播放器
    ui->m_cbAutoExpandOnPlay->setChecked(m_currentPrefs.autoExpandOnPlay);
...
void GlobalSettingsDialog::collectPrefsFromUi() {
    ...
    // 常规
    m_currentPrefs.allowMultipleInstances = ui->m_cbMultiInstance->isChecked();
    m_currentPrefs.autoPlayOnExternalOpen = ui->m_cbAutoPlayExternal->isChecked();
    m_currentPrefs.defaultWorkspace = ui->m_cmbDefaultWorkspace->currentData().toInt();
    ...
    m_currentPrefs.windowType = ui->m_cmbWindow->currentData().value<FFTWindowType>();
    m_currentPrefs.curveType = ui->m_cmbMapping->currentData().value<CurveType>();
    m_currentPrefs.paletteId = ui->m_cmbPalette->currentData().toString();
    m_currentPrefs.paletteInverted = ui->m_cbPaletteInvert->isChecked();
    m_currentPrefs.paletteNegative = ui->m_cbPaletteNegative->isChecked();
    ...
    m_currentPrefs.spectrumProfileDirection = ui->m_cmbProfileDirection->currentData().value<SpectrumProfileDirection>();
    ...
    // 播放器
    m_currentPrefs.autoExpandOnPlay = ui->m_cbAutoExpandOnPlay->isChecked();
