...
PlayerEngine::PlayerEngine(std::shared_ptr<std::atomic<qint64>> sharedPos, QObject* parent) 
    : QObject(parent), m_sharedPos(sharedPos) 
{
    m_posTimer = new QTimer(this);
    m_posTimer->setInterval(10); // 100fps
    connect(m_posTimer, &QTimer::timeout, this, &PlayerEngine::updatePositionAtomic);
...
void PlayerEngine::updatePositionAtomic() {
    if (m_sharedPos) {
        m_sharedPos->store(getCorrectedPositionUSecs(), std::memory_order_relaxed);
    }
}
...
qint64 PlayerEngine::getCorrectedPositionUSecs() {
    ...
    qint64 queuedBytes = m_audioSink->bufferSize() - m_audioSink->bytesFree();
    ...
    if (bytesPerSec > 0) {
        sinkDelay = static_cast<double>(queuedBytes) / bytesPerSec;
    }
...
void PlayerEngine::start(QSharedPointer<XPlayerProvider> provider) {
    stop(); 
    ...
    if (m_posTimer) m_posTimer->start();
}
...
void PlayerEngine::stop() {
    ...
    m_baseOffsetSec = 0.0;
    if (m_sharedPos) m_sharedPos->store(0, std::memory_order_relaxed);
}
...
void PlayerEngine::seek(double seconds) {
    ...
    if (m_sharedPos) m_sharedPos->store(static_cast<qint64>(seconds * 1000000.0), std::memory_order_relaxed);
...
void PlayerEngine::onDefaultDeviceChanged(const QAudioDevice &device) {
    ...
    m_baseOffsetSec = m_currentProvider->currentTime();
    if (!device.isFormatSupported(m_currentFormat)) {
        int nativeRate = m_currentProvider->getNativeSampleRate();
        QAudioFormat preferredFormat;
        preferredFormat.setSampleRate(nativeRate);
        preferredFormat.setChannelCount(2);
