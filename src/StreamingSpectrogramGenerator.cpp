...
StreamingSpectrogramGenerator::StreamingSpectrogramGenerator(QObject *parent) : QObject(parent) {}

QImage StreamingSpectrogramGenerator::generateChunk(
...
{
    if (fftSize <= 0 || targetHeight <= 0 || sampleRate <= 0) return QImage();
    ...
    auto renderWorker = [=, &y_to_bin_map, &colorTable](auto&& flatData) -> QImage {
    ...
    return std::visit(renderWorker, spectrumData);
}